...
static const uint16_t rnd_array[30] = {994, 285, 553, 11,  792, 707, 966, 641, 852, 827, 44,  352, 146, 581, 490,
                                       80,  729, 58,  695, 940, 724, 561, 124, 653, 27,  292, 557, 506, 382, 199};

/**********************
 *      MACROS
 **********************/
lv_ft_info_t ft_yayuan_12;
lv_ft_info_t ft_yayuan_16;
...
lv_obj_t *_lv_demo_music_main_create(lv_obj_t *parent)
{
...
#else
    /*Create a font*/
    ft_yayuan_12.name = "./yayuan.ttf";
    ft_yayuan_12.weight = 12;
    ft_yayuan_12.style = FT_FONT_STYLE_NORMAL;
    lv_ft_font_init(&ft_yayuan_12);

    /*Create a font*/
    lv_ft_info_t ft_yayuan_16;
    ft_yayuan_16.name = "./yayuan.ttf";
    ft_yayuan_16.weight = 16;
    ft_yayuan_16.style = FT_FONT_STYLE_NORMAL;
    lv_ft_font_init(&ft_yayuan_16);
    font_small = ft_yayuan_12.font;
    font_large = ft_yayuan_16.font;
    ...
    // lv_obj_set_size(spectrum_obj, 100, 100);
    ...
    lv_obj_set_grid_cell(spectrum_obj, LV_GRID_ALIGN_STRETCH, 1, 1, LV_GRID_ALIGN_CENTER, 1, 8);
#endif

    sec_counter_timer = lv_timer_create(timer_cb, 1000, NULL);
    lv_timer_pause(sec_counter_timer);

    /*Animate in the content after the intro _time*/
    lv_anim_t a;
    ...
    lv_label_set_text(title, "基于LVGL8的\n音乐播放器");
...
void _lv_demo_music_resume(void)
{
    ...
    pst == _pause ? _demo_music_pause(false) : _demo_music_resume();
    pst = playing;
}
void *get_music_length(void *arg);
void _demo_music_resume()
{
    char cmd[256];
    sprintf(cmd, "{ \"command\": [\"loadfile\", \"./music/%s\"] }\n", _lv_demo_music_get_title(track_id));
    LOG_D("%s", cmd);
    write(fd_mpv, cmd, strlen(cmd));
}

// 2022.06.18 由于最新的代码已丢失，只能重新再修改了一遍了，参考文档 https://mpv.io/manual/master/
// 1.把那个sox去掉了，不需要这个，只是为了获取音乐长度
// 2.改为由mpv事件来驱动gui,以前是定时器
// 3.实现了播放，暂停，上一曲，下一曲，进度条定位等
void *get_music_playback_time(void *arg)
{
    //这是轮询获取播放信息，不推荐
    // char cmd1[] = "{\"command\": [ \"get_property\", \"playback-time\"],\"request_id\":1}\n ";
    // char cmd2[] = "{\"command\": [ \"get_property\", \"percent-pos\"],\"request_id\":2}\n ";
    // char cmd3[] = "{\"command\": [ \"get_property\", \"duration\"],\"request_id\":3}\n ";
    //这种方式是监视指定属性并自动产生event
    char cmd1[] = "{\"command\": [\"observe_property\", 1,\"playback-time\"]}\n ";
    ...
    while (1)
    {
        //这里需要注意，mpv输出的event数据可能是多行的json结果，以\n分隔换行
        memset(buf, 0, sizeof(buf));
        if (read(fd_mpv, buf, sizeof(buf)) > 0)
        {
            // LOG_D("%s len:%d", buf, strlen(buf));
                ...
                if (root != NULL)
                {
                    if (cJSON_HasObjectItem(root, "event"))
                    {
                        ...
                        if (event != NULL)
                        {
                            ...
                            else if (strcmp(event->valuestring, "property-change") == 0)
                            {
                                ...
                                if (id != NULL)
                                {
                                    ...
                                    {
                                        //设置已播放的时间
                                        lv_label_set_text_fmt(time_obj, "%02d:%02d", (int)cjson_obj->valuedouble / 60,
                                                              (int)cjson_obj->valuedouble % 60);
                                        // LOG_D("playback_time: %02d:%02d\n", (int)cjson_obj->valuedouble / 60, (int)cjson_obj->valuedouble % 60);
                                    }
                                    else if (id->valueint == 2)
                                    {
                                        //设置进度条
                                        lv_slider_set_value(slider_obj, (int)cjson_obj->valuedouble, LV_ANIM_OFF);
                                        // LOG_D("percent-pos:%d%%\n", (int)cjson_obj->valuedouble);
                                    }
                                }
                            }
                            else if (strcmp(event->valuestring, "idle") == 0)
                            {
                                _lv_demo_music_album_next(true); //自动加载下一首，默认是列表循环播放
                            }
                        }
                    }
                }
...
void _demo_music_resume(void)
{
    ...
    if (pid == 0)
    {
        ...
        execlp("mpv", "mpv", "--no-video", buf, cmd, NULL);
    ...
    else if (pid > 0)
    {
        ...
        // waitpid(pid, NULL, 1);
        if (pthread_create(&mthread, NULL, get_exec_data, NULL) != 0)
        {
    ...
    // char buf[128];
    // sLOG_D(buf, "play \"./music/%s\"", _lv_demo_music_get_title(track_id)); //注意文件名空格
    // FILE *fp = popen(buf, "r");
    // if (ferror(fp))
    // {
    //     fLOG_D(stderr, "error\n");
...
void _lv_demo_music_pause(void)
{
    ...
    _demo_music_pause(true);
}
void _demo_music_pause(bool sta)
{
    char cmd[256];
...
#endif
    static const lv_coord_t grid_col[] = {LV_GRID_FR(1), LV_GRID_FR(2), LV_GRID_FR(3), LV_GRID_FR(3),
                                          LV_GRID_FR(3), LV_GRID_FR(2), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
...
static lv_obj_t *create_handle(lv_obj_t *parent)
{
    ...
    lv_label_set_text(handle_label, "播放列表");
...
static void track_load(uint32_t id)
{
    ...
    LOG_D("music id:%d\n", track_id + 1);
    LOG_D("music name:%s\n", _lv_demo_music_get_title(track_id));
    // LOG_D("music length:%02d:%02d\n", _lv_demo_music_get_track_length(track_id) / 60, _lv_demo_music_get_track_length(track_id) % 60);
...
static void play_event_click_cb(lv_event_t *e)
{
    ...
    lv_obj_has_state(obj, LV_STATE_CHECKED) ? _lv_demo_music_resume() : _lv_demo_music_pause();
}
...
{
    // _time++;
    // lv_label_set_text_fmt(time_obj, "%02d:%02d", _time / 60, _time % 60);
    // // lv_slider_set_value(slider_obj, _time, LV_ANIM_ON);
    // // LOG_D("music time:%d _time:%d\n", _lv_demo_music_get_track_length(track_id), _time);
    // if (_time >= _lv_demo_music_get_track_length(track_id))
    // {
    //     LOG_D("next music\n");
    //     _lv_demo_music_album_next(true);
    // }
...
static void slider_event_cb(lv_event_t *e)
{
    ...
    // lv_slider_set_value(slider_obj, _time, LV_ANIM_ON);
    char cmd[256];
    sprintf(cmd, "{ \"command\": [\"seek\", %d,\"absolute-percent\"] }\n", pos);
    write(fd_mpv, cmd, strlen(cmd));
    // _time = (pos * _lv_demo_music_get_track_length(track_id)) / 100;
    // lv_label_set_text_fmt(time_obj, "%02d:%02d", _time / 60, _time % 60);
}
static void spectrum_end_cb(lv_anim_t *a)
{
