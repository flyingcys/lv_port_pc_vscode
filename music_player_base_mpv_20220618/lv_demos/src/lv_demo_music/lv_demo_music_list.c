 ...
 *      TYPEDEFS
 **********************/
extern lv_ft_info_t ft_yayuan_12;
extern lv_ft_info_t ft_yayuan_16;
/**********************
 *  STATIC PROTOTYPES
 **********************/
...
lv_obj_t *_lv_demo_music_list_create(lv_obj_t *parent)
{
...
#else
    static const lv_coord_t grid_rows[] = {22, 0, LV_GRID_TEMPLATE_LAST};
#endif
    ...
    for (track_id = 0; track_id < music_num; track_id++)
    {
...
static lv_obj_t *add_list_btn(lv_obj_t *parent, uint32_t track_id)
{
    ...
    lv_label_set_text_fmt(index_label, "  %03d.  ", track_id + 1);
    lv_obj_set_grid_cell(index_label, LV_GRID_ALIGN_START, 0, 1, LV_GRID_ALIGN_CENTER, 0, 1);
    lv_obj_add_style(index_label, &style_title, 0);

    lv_obj_t *title_label = lv_label_create(btn);
    lv_label_set_text(title_label, title);
    lv_obj_set_grid_cell(title_label, LV_GRID_ALIGN_START, 1, 1, LV_GRID_ALIGN_SPACE_BETWEEN, 0, 1);
    lv_obj_add_style(title_label, &style_title, 0);
    lv_obj_set_width(title_label, lv_pct(76));
