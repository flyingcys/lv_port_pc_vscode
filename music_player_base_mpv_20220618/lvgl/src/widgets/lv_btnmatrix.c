...
const lv_obj_class_t lv_btnmatrix_class = {.constructor_cb = lv_btnmatrix_constructor,
...
static void lv_btnmatrix_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
    ...
    else if (code == LV_EVENT_PRESSED)
    {
        ...
        if (indev_type == LV_INDEV_TYPE_POINTER || indev_type == LV_INDEV_TYPE_BUTTON)
        {
            ...
            {
                if (button_is_inactive(btnm->ctrl_bits[btn_pr]) == false && button_is_hidden(btnm->ctrl_bits[btn_pr]) == false)
                {
        ...
        {
            if (button_is_click_trig(btnm->ctrl_bits[btnm->btn_id_sel]) == false && button_is_inactive(btnm->ctrl_bits[btnm->btn_id_sel]) == false &&
    ...
    else if (code == LV_EVENT_PRESSING)
    {
        ...
        if (btn_pr != btnm->btn_id_sel)
        {
            ...
            if (btn_pr != LV_BTNMATRIX_BTN_NONE && button_is_inactive(btnm->ctrl_bits[btn_pr]) == false &&
    ...
    else if (code == LV_EVENT_RELEASED)
    {
        if (btnm->btn_id_sel != LV_BTNMATRIX_BTN_NONE)
        {
            ...
            if (button_is_checkable(btnm->ctrl_bits[btnm->btn_id_sel]) && !button_is_inactive(btnm->ctrl_bits[btnm->btn_id_sel]))
            {
            ...
            if (button_is_click_trig(btnm->ctrl_bits[btnm->btn_id_sel]) == true && button_is_inactive(btnm->ctrl_bits[btnm->btn_id_sel]) == false &&
                button_is_hidden(btnm->ctrl_bits[btnm->btn_id_sel]) == false)
            {
                uint32_t b = btnm->btn_id_sel;
                res = lv_event_send(obj, LV_EVENT_VALUE_CHANGED, &b);
    ...
    else if (code == LV_EVENT_LONG_PRESSED_REPEAT)
    {
        if (btnm->btn_id_sel != LV_BTNMATRIX_BTN_NONE)
        {
            if (button_is_repeat_disabled(btnm->ctrl_bits[btnm->btn_id_sel]) == false &&
                button_is_inactive(btnm->ctrl_bits[btnm->btn_id_sel]) == false && button_is_hidden(btnm->ctrl_bits[btnm->btn_id_sel]) == false)
            {
                uint32_t b = btnm->btn_id_sel;
                res = lv_event_send(obj, LV_EVENT_VALUE_CHANGED, &b);
                if (res != LV_RES_OK)
    ...
    else if (code == LV_EVENT_KEY)
    {
        ...
        else if (c == LV_KEY_DOWN)
        {
            ...
            {
                uint16_t area_below;
                lv_coord_t pr_center = btnm->button_areas[btnm->btn_id_sel].x1 + (lv_area_get_width(&btnm->button_areas[btnm->btn_id_sel]) >> 1);
                ...
                for (area_below = btnm->btn_id_sel; area_below < btnm->btn_cnt; area_below++)
                {
                    if (btnm->button_areas[area_below].y1 > btnm->button_areas[btnm->btn_id_sel].y1 &&
                        pr_center >= btnm->button_areas[area_below].x1 && pr_center <= btnm->button_areas[area_below].x2 + col_gap &&
                        button_is_inactive(btnm->ctrl_bits[area_below]) == false && button_is_hidden(btnm->ctrl_bits[area_below]) == false)
                    {
                        break;
                    }
                }
            ...
            else
            {
                ...
                for (area_above = btnm->btn_id_sel; area_above >= 0; area_above--)
                {
                    if (btnm->button_areas[area_above].y1 < btnm->button_areas[btnm->btn_id_sel].y1 &&
                        pr_center >= btnm->button_areas[area_above].x1 - col_gap && pr_center <= btnm->button_areas[area_above].x2 &&
                        button_is_inactive(btnm->ctrl_bits[area_above]) == false && button_is_hidden(btnm->ctrl_bits[area_above]) == false)
                    {
                        break;
                    }
                }
...
#endif
        lv_point_t txt_size;
        lv_txt_get_size(&txt_size, txt, font, letter_space, line_space, lv_area_get_width(&area_obj), draw_label_dsc_act.flag);
...
static bool button_is_hidden(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_HIDDEN) ? true : false; }

static bool button_is_checked(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_CHECKED) ? true : false; }

static bool button_is_repeat_disabled(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_NO_REPEAT) ? true : false; }

static bool button_is_inactive(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_DISABLED) ? true : false; }

static bool button_is_click_trig(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_CLICK_TRIG) ? true : false; }

static bool button_is_checkable(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_CHECKABLE) ? true : false; }

static bool button_get_checked(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_CHECKED) ? true : false; }

static bool button_is_recolor(lv_btnmatrix_ctrl_t ctrl_bits) { return (ctrl_bits & LV_BTNMATRIX_CTRL_RECOLOR) ? true : false; }
/**
...
static uint16_t get_button_from_point(lv_obj_t *obj, lv_point_t *p)
{
    ...
    for (i = 0; i < btnm->btn_cnt; i++)
    {
        ...
        if (btn_area.x2 >= w - pright - 2)
            btn_area.x2 += obj_cords.x1 + LV_MIN(pright, BTN_EXTRA_CLICK_AREA_MAX); /*-2 for rounding error*/
        else
            btn_area.x2 += obj_cords.x1 + pcol;

        if (btn_area.y2 >= h - pbottom - 2)
            btn_area.y2 += obj_cords.y1 + LV_MIN(pbottom, BTN_EXTRA_CLICK_AREA_MAX); /*-2 for rounding error*/
        else
