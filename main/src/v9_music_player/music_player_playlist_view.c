...
void music_player_playlist_view_build(lv_obj_t * parent, music_player_app_t * app)
{
    ...
    lv_label_set_text(lv_label_create(head), "PLAYLIST STACK");
    lv_label_set_text(lv_label_create(head), "Playlists");
    lv_label_set_text(lv_label_create(head), "Show personal and suggested lists in a lighter card rhythm.");
    ...
    lv_label_set_text(lv_label_create(banner), "Your lists today");
    lv_label_set_text(lv_label_create(banner), "Keep structure stable before deeper interaction logic.");
    ...
    for(size_t i = 0; i < music_player_playlist_count(); ++i) {
        const music_player_playlist_item_t * item = &music_player_playlist_items()[i];
    ...
    lv_label_set_text(lv_label_create(side), "List notes");
    lv_label_set_text(lv_label_create(side), "Pinned set");
    lv_label_set_text(lv_label_create(side), "Smart mix");
    lv_label_set_text(lv_label_create(side), "Shared entry later");
    LV_UNUSED(app);
}
