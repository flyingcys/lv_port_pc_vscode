...
static lv_timer_t * g_pending_restore_timers[3];
...
static lv_obj_t * fruit_ninja_create_internal(lv_obj_t * parent, int32_t phys_w, int32_t phys_h)
{
    char cwd[512];
    ...
    create_static_scene(&g_game, parent, phys_w, phys_h);
...
{
    if(parent == NULL || screen_w <= 0 || screen_h <= 0) {
        return NULL;
    }

    return fruit_ninja_create_internal(parent, screen_w, screen_h);
}

void fruit_ninja_start(void)
{
    lv_display_t * disp = lv_display_get_default();
    int32_t phys_w = (int32_t)lv_display_get_horizontal_resolution(disp);
    int32_t phys_h = (int32_t)lv_display_get_vertical_resolution(disp);
    lv_obj_t * screen = fruit_ninja_create_internal(NULL, phys_w, phys_h);
...
{
    if(g_game.update_timer != NULL) {
        lv_timer_delete(g_game.update_timer);
        g_game.update_timer = NULL;
    }
    ...
    for(uint32_t i = 0; i < 3; i++) {
        if(g_pending_restore_timers[i] != NULL) {
            lv_timer_delete(g_pending_restore_timers[i]);
            g_pending_restore_timers[i] = NULL;
        }
    }
    if(g_game.screen != NULL) {
        lv_obj_delete(g_game.screen);
    }
    fruit_ninja_audio_shutdown();
    memset(&g_game, 0, sizeof(g_game));
}

void fruit_ninja_register_start_timer(lv_timer_t * timer)
{
    g_pending_start_timer = timer;
}

void fruit_ninja_register_restore_timer(lv_timer_t * timer)
{
    for(uint32_t i = 0; i < 3; i++) {
        if(g_pending_restore_timers[i] == NULL) {
            g_pending_restore_timers[i] = timer;
            return;
        }
    }
}

void fruit_ninja_clear_timer(lv_timer_t * timer)
{
    if(g_pending_start_timer == timer) {
        g_pending_start_timer = NULL;
    }
    for(uint32_t i = 0; i < 3; i++) {
        if(g_pending_restore_timers[i] == timer) {
            g_pending_restore_timers[i] = NULL;
        }
    }
}
