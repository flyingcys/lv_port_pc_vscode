...
{
	memcpy(&cpu->mem[0x8000], cpu->cpu_mapper_io->prg_rom->data + ((prg_bank_offset) * (kib_size)), kib_size);
}

void set_prg_rom_bank_2(Cpu6502 *cpu, unsigned prg_bank_offset)
{
	memcpy(&cpu->mem[0xC000], cpu->cpu_mapper_io->prg_rom->data + ((prg_bank_offset) * (16 * KiB)), 16 * KiB);
}

void set_chr_bank_1(Cpu6502 *cpu, unsigned chr_bank_offset, unsigned kib_size)
{
	memcpy(&cpu->cpu_ppu_io->vram[0x0000], cpu->cpu_mapper_io->chr->data + ((chr_bank_offset) * (kib_size)), kib_size);
}

void set_chr_bank_2(Cpu6502 *cpu, unsigned chr_bank_offset)
{
	memcpy(&cpu->cpu_ppu_io->vram[0x1000], cpu->cpu_mapper_io->chr->data + ((chr_bank_offset) * (4 * KiB)), 4 * KiB);
}
...
void change_nt_mirroring(Cpu6502 *cpu)
{
	...
	if ((*(cpu->cpu_ppu_io->mirroring) != 2) || (*(cpu->cpu_ppu_io->mirroring) != 3))
	{
...
void mmc1_reg_write(Cpu6502 *cpu, uint16_t addr, uint8_t val)
{
	...
	if (write_count == 5)
	{
		...
		{ // else (addr >= 0xE000 && addr <= 0xFFFF) --> reg 3
