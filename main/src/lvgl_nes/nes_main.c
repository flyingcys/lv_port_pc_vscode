...
static void slider_event_cb(lv_event_t * e)
{
    ...
    if(strcmp(txt, "Volume") == 0) {
        ...
        lv_100ask_nes_set_zoom(lv_nes, lv_slider_get_value(slider));
    }
    else if(strcmp(txt, "Velocity") == 0) {
        lv_100ask_nes_set_speed(lv_nes, lv_slider_get_value(slider));
    }
}
...
{
    lv_nes = lv_100ask_nes_create(lv_screen_active());
    lv_obj_center(lv_nes);

    init_my_nes_front_end(lv_nes);
