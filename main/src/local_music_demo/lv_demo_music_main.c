...
static uint32_t get_track_count(void);
static void stop_start_anim(lv_timer_t * t);
static void start_spectrum_anim(void);
...
static bool seeking;
static bool track_switching;
static uint32_t seek_target_ms;
...
void lv_demo_music_album_next(bool next)
{
    ...
    uint32_t track_cnt = get_track_count();

    if(track_cnt == 0U) return;
    if(next) {
        id++;
...
static lv_obj_t * create_icon_box(lv_obj_t * parent)
{
    ...
    lv_obj_add_event_cb(icon, mute_click_event_cb, LV_EVENT_CLICKED, NULL);
    mute_btn = icon;
    icon = lv_image_create(cont);
...
static void play_event_click_cb(lv_event_t * e)
{
    ...
    if(lv_obj_has_state(obj, LV_STATE_CHECKED)) {
        if(music_player_get_count() > 0U) music_player_resume();
        lv_demo_music_resume();
    }
    else {
        if(music_player_get_count() > 0U) music_player_pause();
        lv_demo_music_pause();
    }
}
...
{
    LV_UNUSED(e);
    if(music_player_get_count() > 0U) {
        _lv_demo_music_begin_track_switch();
        music_player_prev();
    }
    else {
...
static void slider_event_cb(lv_event_t * e)
{
    ...
    if(music_player_get_count() == 0U) return;
    if(music_player_get_duration_ms() == 0U) return;
    if(track_switching) return;

    position_s = (uint32_t)lv_slider_get_value(lv_event_get_target(e));
    lv_label_set_text_fmt(time_obj, "%"LV_PRIu32":%02"LV_PRIu32, position_s / 60U, position_s % 60U);

    if(code == LV_EVENT_VALUE_CHANGED) {
        seeking = true;
        seek_target_ms = position_s * 1000U;
    }
    else if(code == LV_EVENT_RELEASED) {
        seek_target_ms = position_s * 1000U;
        music_player_seek(seek_target_ms);
    }
}
...
static void timer_cb(lv_timer_t * t)
{
    ...
    if(music_player_get_count() > 0U) {
        ...
        if(seeking) {
            ...
            position_s = seek_target_ms / 1000U;
            real_ms = music_player_get_position_ms();
            if(real_ms + 1500U >= seek_target_ms) seeking = false;
            lv_label_set_text_fmt(time_obj, "%"LV_PRIu32":%02"LV_PRIu32, position_s / 60U, position_s % 60U);
            lv_slider_set_value(slider_obj, position_s, LV_ANIM_OFF);
            return;
        }

        position_s = music_player_get_position_ms() / 1000U;
        duration_s = music_player_get_duration_ms() / 1000U;
        if(duration_s > 0U) lv_slider_set_range(slider_obj, 0, duration_s);
        lv_label_set_text_fmt(time_obj, "%"LV_PRIu32":%02"LV_PRIu32, position_s / 60U, position_s % 60U);
        lv_slider_set_value(slider_obj, position_s, LV_ANIM_ON);
        return;
    }
...
static uint32_t get_track_count(void)
{
    uint32_t count = (uint32_t)music_player_get_count();
    return count > 0U ? count : ACTIVE_TRACK_CNT;
}

static void update_mute_visual(void)
{
    if(!mute_btn) return;
    lv_obj_set_style_image_opa(mute_btn, music_player_is_muted() ? LV_OPA_50 : LV_OPA_COVER, 0);
}
