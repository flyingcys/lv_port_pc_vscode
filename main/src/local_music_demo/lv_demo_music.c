...
{
    if(music_player_get_count() > 0U) return music_player_get_title(track_id);
...
const char * lv_demo_music_get_artist(uint32_t track_id)
{
    if(music_player_get_count() > 0U) {
        return music_player_is_live(track_id) ? "Live stream" : "Local file";
    }
...
{
    if(music_player_get_count() > 0U) {
        return music_player_get_track_duration_ms(track_id) / 1000U;
    }
    if(track_id >= sizeof(time_list) / sizeof(time_list[0])) return 0;
    return time_list[track_id];
}

lv_obj_t * _lv_demo_music_get_list(void)
{
    return list;
}
