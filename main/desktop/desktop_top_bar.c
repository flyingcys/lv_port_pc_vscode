...
static void top_bar_apply_slot(lv_obj_t * slot, lv_obj_t * label, topbar_slot_type_t type, const char * text,
                               lv_align_t align);
static void top_bar_apply_page_mode(desktop_top_bar_t * top_bar, topbar_page_mode_t page_mode);
...
static void top_bar_update_current_time(desktop_top_bar_t * top_bar);
static uint32_t top_bar_get_initial_period_ms(void);
static void top_bar_configure_root(desktop_top_bar_t * top_bar, lv_obj_t * parent);
static void top_bar_configure_slots(desktop_top_bar_t * top_bar);
...
{
    desktop_top_bar_t * top_bar = lv_malloc_zeroed(sizeof(*top_bar));
    ...
    /* 初始化右槽图标文本 */
    lv_label_set_text(top_bar->signal_label,  DESKTOP_GLYPH_SIGNAL);
    lv_label_set_text(top_bar->wifi_label,    DESKTOP_GLYPH_WIFI);
    lv_label_set_text(top_bar->battery_label, DESKTOP_GLYPH_BATTERY_FULL);
    lv_label_set_text(top_bar->battery_pct_label, "100%");

    /* 初始化时钟 */
    desktop_top_bar_set_time(top_bar, "--:--");
...
static void top_bar_delete_cb(lv_event_t * e)
{
    desktop_top_bar_t * top_bar = lv_event_get_user_data(e);
...
static void top_bar_minute_timer_cb(lv_timer_t * timer)
{
    desktop_top_bar_t * top_bar = lv_timer_get_user_data(timer);
...
static const char * top_bar_wifi_glyph(wifi_state_t state)
{
    switch(state) {
        ...
        case WIFI_STATE_DISCONNECTED:
            return DESKTOP_GLYPH_WIFI;   /* 淡化显示（opa 由调用方设置）*/
        case WIFI_STATE_WEAK:
        case WIFI_STATE_NORMAL:
        case WIFI_STATE_STRONG:
        default:
            return DESKTOP_GLYPH_WIFI;
    }
}
...
static void top_bar_update_current_time(desktop_top_bar_t * top_bar)
{
    ...
    desktop_top_bar_set_time(top_bar, time_buf);
}
...
static void top_bar_configure_root(desktop_top_bar_t * top_bar, lv_obj_t * parent)
{
    ...
    /* 文字色：主题白色 */
    lv_obj_set_style_text_color(top_bar->root, desktop_theme()->text_primary, 0);
...
static lv_obj_t * top_bar_create_clock_label(lv_obj_t * parent, lv_align_t align)
{
    ...
    lv_obj_set_style_text_color(label, desktop_theme()->text_primary, 0);
