...
#define DESKTOP_DOT_INACTIVE_OPA 96   /* 分页点非活跃透明度 */

void desktop_make_decorative(lv_obj_t * obj){
    lv_obj_remove_flag(obj, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_remove_flag(obj, LV_OBJ_FLAG_SCROLLABLE);
}

lv_obj_t * desktop_widget_app_tile(lv_obj_t * parent, const desktop_app_t * app){
    const desktop_metrics_t * m = desktop_metrics();
    const desktop_theme_t * th = desktop_theme();
    int32_t col_w = desktop_grid_col_w();
    ...
    desktop_make_decorative(lbl);
    return tile;
}

lv_obj_t * desktop_widget_glass_panel(lv_obj_t * parent, int32_t w, int32_t h){
    const desktop_theme_t * th = desktop_theme();
    ...
    desktop_make_decorative(p);
    return p;
}

lv_obj_t * desktop_widget_toggle(lv_obj_t * parent, const char * glyph, bool on){
    const desktop_metrics_t * m = desktop_metrics(); const desktop_theme_t * th = desktop_theme();
    ...
    lv_label_set_text(g, glyph); lv_obj_center(g); desktop_make_decorative(g);
    return b;
}

lv_obj_t * desktop_widget_slider(lv_obj_t * parent, const char * glyph, int32_t val){
    const desktop_metrics_t * m = desktop_metrics(); const desktop_theme_t * th = desktop_theme();
    lv_obj_t * row = lv_obj_create(parent); lv_obj_remove_style_all(row);
    lv_obj_set_size(row, LV_PCT(100), LV_SIZE_CONTENT);
    lv_obj_set_flex_flow(row, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(row, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_column(row, 10, 0); desktop_make_decorative(row);
    lv_obj_t * g = lv_label_create(row);
    lv_obj_set_style_text_font(g, m->font_glyph, 0);
    lv_obj_set_style_text_color(g, th->text_primary, 0);
    lv_label_set_text(g, glyph); desktop_make_decorative(g);
...
lv_obj_t * desktop_widget_dots(lv_obj_t * parent, uint32_t count, uint32_t active){
    const desktop_theme_t * th = desktop_theme();
    lv_obj_t * row = lv_obj_create(parent); lv_obj_remove_style_all(row);
    lv_obj_set_size(row, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    lv_obj_set_flex_flow(row, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(row, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_column(row, 8, 0); desktop_make_decorative(row);
    for(uint32_t i=0;i<count;i++){
        ...
        lv_obj_set_style_bg_opa(d, a?LV_OPA_COVER:DESKTOP_DOT_INACTIVE_OPA, 0);
        desktop_make_decorative(d);
    }
    return row;
}

void desktop_widget_dots_set_active(lv_obj_t * dots, uint32_t active){
    uint32_t n = lv_obj_get_child_count(dots);
    for(uint32_t i=0;i<n;i++){ lv_obj_t*d=lv_obj_get_child(dots,i); bool a=(i==active);
        lv_obj_set_width(d, a?18:8); lv_obj_set_style_bg_opa(d, a?LV_OPA_COVER:DESKTOP_DOT_INACTIVE_OPA, 0); }
}

lv_obj_t * desktop_widget_panel_handle(lv_obj_t * parent){
    const desktop_metrics_t * m = desktop_metrics();
    const desktop_theme_t * th = desktop_theme();
