...
void log_cpu_info(Cpu6502 *cpu)
{
	...
	{ // first cycle = +1 cycles due to the tick() after the instruction executes
...
{
	if (carry_out)
	{					  // Carry out = result >> 8 (9th bit in ADC / SBC calc
		cpu->P |= FLAG_C; /* Set C */
	}
	else
	{
...
void execute_DMA(Cpu6502 *cpu)
{
	...
	if (((cpu->cycle - 1) & 1) && cycles_left == 514)
	{
		cycles_left += 1;
	}
