...
日历部件以年/月视图展示日期，可配合事件回调实现日期选择、高亮提示等功能。

## 25.1 主要功能与 API

```c
lv_obj_t * lv_calendar_create(lv_obj_t * parent);
void lv_calendar_set_today_date(lv_obj_t * calendar, const lv_calendar_date_t * today);
void lv_calendar_set_showed_date(lv_obj_t * calendar, const lv_calendar_date_t * showed);
void lv_calendar_set_highlighted_dates(lv_obj_t * calendar,
                                       const lv_calendar_date_t * highlighted,
                                       uint16_t date_num);
lv_result_t lv_calendar_get_pressed_date(lv_obj_t * calendar, lv_calendar_date_t * date);
```

- `lv_calendar_date_t` 结构体包含 `year`、`month`、`day` 字段，用于设置/读取日期。
- 高亮日期通常用于节假日、提醒等场景。
...
static void calendar_event_cb(lv_event_t * e)
{
    ...
    if(lv_event_get_code(e) == LV_EVENT_VALUE_CHANGED &&
       lv_calendar_get_pressed_date(calendar, &date) == LV_RESULT_OK) {
        char buf[16];
...
void lv_example_calendar_basic(void)
{
    ...
    lv_calendar_date_t today = {2024, 4, 7};
    ...
    lv_calendar_set_highlighted_dates(calendar, highlighted,
                                      sizeof(highlighted) / sizeof(highlighted[0]));

    lv_obj_t * label = lv_label_create(lv_screen_active());
    lv_obj_align(label, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
    lv_label_set_text(label, "Wait input...");

    lv_obj_add_event_cb(calendar, calendar_event_cb,
...
1. 创建日历并设置“今天”与默认显示的年月。
2. 通过 `lv_calendar_set_highlighted_dates` 标记关键日期（5、6 号）。
3. 添加事件回调，当选中日期变化时读取日期并显示在标签上。

## 25.3 小结

- 日历部件通过 `lv_calendar_date_t` 与一组接口实现日期的设置/获取。
- 可结合标签、弹框等部件，实现日期选择器或提醒功能。
- 若需更多控制（如自定义标题、导航按钮），可配合 `lv_calendar_header_checkbox`、`lv_calendar_header_dropdown` 等扩展部件。
# 第二十六章 图表部件（lv_chart）

图表部件用于对数据进行可视化展示，支持折线图、条形图、散点图等模式。为了便于理解，本章内容按照“部件组成 → 关键 API → 示例代码”的顺序进行说明。

## 26.1 图表的组成

| 组成部分            | 说明                                                         |
|---------------------|--------------------------------------------------------------|
| `LV_PART_MAIN`      | 图表背景、分隔线及坐标轴区域。                               |
| `LV_PART_INDICATOR` | 折线图/散点图上的点、柱状图柱体等“数据指示器”。              |
| `LV_PART_ITEMS`     | 专用于条形图/散点图等特殊项目的样式扩展。                   |
| `LV_PART_TICKS`     | 刻度线与刻度文本。                                           |
| `LV_PART_CURSOR`    | 游标样式（若启用）。                                         |
| `LV_PART_SCROLLBAR` | 缩放后出现滚动区域时的滚动条。                               |
...
- **管理数据序列**
  ...
  lv_chart_set_ext_y_array(chart, series, y_buffer);
  ```
- **更新数据显示**
  ```c
  lv_chart_set_next_value(chart, series, new_point);
...
- **辅助功能（刻度、游标、缩放）**
  ...
  lv_chart_set_zoom_x(chart, LV_SCALE_NONE * 2);
  lv_chart_set_zoom_y(chart, LV_SCALE_NONE * 2);
  ```

## 26.3 示例：折线图与缩放

下面示例展示如何创建一个折线图、写入测试数据，并通过滑块控制缩放。示例使用 LVGL 9.x API，可直接在 PC 模拟器运行。

```c
#define POINT_CNT 64

static lv_coord_t chart_data[POINT_CNT];

static void slider_x_event_cb(lv_event_t * e)
{
    lv_obj_t * slider = lv_event_get_target(e);
    lv_obj_t * chart  = lv_event_get_user_data(e);
    lv_chart_set_zoom_x(chart, lv_slider_get_value(slider));
}
...
static void slider_y_event_cb(lv_event_t * e)
{
    lv_obj_t * slider = lv_event_get_target(e);
    lv_obj_t * chart  = lv_event_get_user_data(e);
    lv_chart_set_zoom_y(chart, lv_slider_get_value(slider));
}

void lv_example_chart_zoom(void)
{
    ...
    for(uint32_t i = 0; i < POINT_CNT; i++) {
        chart_data[i] = lv_trigo_sin(i * 360 / POINT_CNT) / 40 + 20;
    }
    lv_chart_set_ext_y_array(chart, series, chart_data);
    ...
    lv_slider_set_range(slider_x, LV_SCALE_NONE, LV_SCALE_NONE * 4);
    lv_obj_set_width(slider_x, lv_obj_get_width(chart));
    lv_obj_align(slider_x, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
    lv_obj_add_event_cb(slider_x, slider_x_event_cb, LV_EVENT_VALUE_CHANGED, chart);
    lv_slider_set_value(slider_x, LV_SCALE_NONE, LV_ANIM_OFF);

    lv_obj_t * slider_y = lv_slider_create(lv_screen_active());
    lv_slider_set_range(slider_y, LV_SCALE_NONE, LV_SCALE_NONE * 4);
    lv_obj_set_height(slider_y, lv_obj_get_height(chart));
    lv_obj_align(slider_y, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
    lv_obj_add_event_cb(slider_y, slider_y_event_cb, LV_EVENT_VALUE_CHANGED, chart);
    lv_slider_set_value(slider_y, LV_SCALE_NONE, LV_ANIM_OFF);
}
```

步骤概述：

1. 创建图表并设置类型、点数和 Y 轴范围。  
2. 准备测试数据并绑定到序列。  
3. 通过 `lv_chart_set_axis_tick` 与 `lv_chart_add_cursor` 添加刻度和游标。  
4. 使用两个滑块分别控制 X、Y 方向缩放，实现交互式查看。

## 26.4 小结

- `lv_chart_set_type` 决定绘制模式，常用取值为 `LV_CHART_TYPE_LINE`、`LV_CHART_TYPE_BAR`。  
- 每个数据序列使用 `lv_chart_series_t` 管理，结合 `lv_chart_set_point_count`、`lv_chart_set_ext_y_array` 可轻松更新数据。  
- 辅助函数（刻度、游标、缩放）可显著提升可读性，根据需求组合使用即可。
...
上述源码中，定时器中断回调函数调用了LVGL的lv_tick_inc函数，该函数可以让LVGL内部的时基参数加1（入口参数为1的情况下）。
...
缓冲区初始化完成后，LVGL 9.x 使用显示对象 (`lv_display_t`) 来管理驱动，而不再通过 `lv_disp_drv_t` 手动注册。典型的注册流程如下所示：
...
{
    lcd_transfer(area, px_map);      /* 用户实现的数据搬运 */
    lv_display_flush_ready(disp);    /* 别忘记通知 LVGL 刷新完成 */
}
```

上述代码可以总结为以下步骤：

第一步：调用 `lv_display_create()` 创建显示对象并设置分辨率。

第二步：通过 `lv_display_set_flush_cb()`、`lv_display_set_color_format()` 等函数完成基础配置。

第三步：使用 `lv_display_set_buffers()` 或 `lv_display_set_draw_buffers()` 绑定绘图缓冲区。

第四步：根据需要调用 `lv_display_set_default()`、`lv_display_get_next()` 管理多显示场景。
...
1. 完成底层显示控制器和背光等外设的初始化步骤（例如上文 `board_display_init()`）。
2. 在 `disp_flush()` 中触发 DMA 或总线传输，把 `px_map` 数据搬运到帧缓存或显存中。
3. 当传输结束后调用 `lv_display_flush_ready()` 告知 LVGL 可以继续渲染下一片区域。
...
当硬件排布无法直接调整方向时，可以使用 LVGL 的旋转接口实现软件层面的方向修正：

```c
lv_display_set_rotation(disp, LV_DISPLAY_ROTATION_180);
```

若需要启用矩阵旋转（适用于任意角度或特殊变换），可以调用：
...
① `LV_DISPLAY_ROTATION_0`：无需旋转  \n② `LV_DISPLAY_ROTATION_90`：顺时针旋转 90 度  \n③ `LV_DISPLAY_ROTATION_180`：顺时针旋转 180 度  \n④ `LV_DISPLAY_ROTATION_270`：顺时针旋转 270 度

需要注意，纯软件旋转会带来额外的内存和时间开销，若硬件控制器支持方向配置，应优先使用硬件能力实现。
...
输入设备可以让用户与系统进行交互，在 LVGL 9.x 中，触摸屏、编码器、键盘等设备通过 `lv_indev_create()` 和相关的 setter 函数完成注册。
...
lv_indev_set_display(indev_touchpad, disp);  /* 可选：指定归属的显示对象 */
```
...
整个流程包含三个主要步骤：创建输入设备、设置类型与读取回调、按需绑定显示或用户数据。不同类型的输入设备在配置细节上有所差异，但注册方式保持一致。
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td>lv_indev_create()</td><td>创建输入设备对象</td></tr><tr><td>lv_indev_delete()</td><td>删除输入设备</td></tr><tr><td>lv_indev_set_type()</td><td>设置输入设备类型</td></tr><tr><td>lv_indev_set_read_cb()</td><td>设置读取回调函数</td></tr><tr><td>lv_indev_set_display()</td><td>绑定目标显示对象</td></tr><tr><td>lv_indev_set_user_data()</td><td>保存用户自定义数据</td></tr><tr><td>lv_indev_enable()</td><td>启用或禁用输入设备</td></tr><tr><td>lv_indev_get_next()</td><td>遍历系统中的下一个输入设备</td></tr><tr><td>lv_indev_active()</td><td>获取当前正在处理的输入设备</td></tr><tr><td>lv_indev_read()</td><td>主动触发一次读取</td></tr></table>

以下对几个关键函数进行说明：

# (1) lv_indev_create 函数
...
返回值：成功时返回新创建的输入设备指针，失败时返回 NULL。

# (2) lv_indev_set_type 函数
...
表 5.4.2.2 lv_indev_set_type 函数形参描述  

<table><tr><td>参数</td><td>描述</td></tr><tr><td>indev</td><td>待配置的输入设备对象</td></tr><tr><td>indev_type</td><td>输入设备类型（如 LV_INDEV_TYPE_POINTER）</td></tr></table>

返回值：无。

# (3) lv_indev_set_read_cb 函数
...
表 5.4.2.3 lv_indev_set_read_cb 函数形参描述  

<table><tr><td>参数</td><td>描述</td></tr><tr><td>indev</td><td>待配置的输入设备对象</td></tr><tr><td>read_cb</td><td>用户实现的读取回调函数</td></tr></table>

返回值：无。

# (4) lv_indev_delete 函数
...
<table><tr><td>参数</td><td>描述</td></tr><tr><td>indev</td><td>要删除的输入设备对象</td></tr></table>
...
该函数可计算自上一次时间戳以来经过的毫秒数，函数原型如下：

```c
uint32_t lv_tick_elaps(uint32_t prev_tick);
```

表 5.5.1 lv_tick_elaps 函数参数描述  

<table><tr><td>参数</td><td>描述</td></tr><tr><td>prev_tick</td><td>上一次记录的毫秒时间戳</td></tr></table>

返回值：从 `prev_tick` 到当前的毫秒差值。

# 5.6 LVGL 任务处理

LVGL的任务处理函数（lv_timer_handler）类似于RTOS切换任务的任务调度函数。lv_timer_handler函数对于定时的要求并不严格，一般来说，用户只需要确保在5毫秒以内调用

一次该函数即可，以保持系统响应的速度。值得注意的是，LVGL的任务处理并不是抢占式的，而是采用轮询的方式。

要处理LVGL的任务或者回调函数，用户只需要将lv_timer_handler函数定时调用即可，该函数可放在以下地方：
...
while(1) {
    if(lv_display_get_inactive_time(NULL) < 1000) {
        ...
        timer_stop();        /* 停止驱动 lv_tick_inc 的定时器 */
        sleep();             /* 进入用户自定义的低功耗模式 */
    }
...
如果需要通过输入唤醒（按键、触摸等），可以在检测到活动时执行以下步骤：
...
如果用户需要使用实际任务或线程，则需要一个互斥锁，以防止任务翻转，该互斥锁应在lv_timer_handler函数之前被调用，并在它之后释放，除此之外，用户在调用LVGL相关的函数和代码时，其相关的任务和线程中也需要使用相同的互斥锁。
...
用户应该尽量避免在中断里调用LVGL函数（lv_tick_inc和lv_display_flush_ready函数除外），如果必须执行此操作，则需要在lv_timer_handler函数运行时，禁用相关的中断。
...
在上述源码中，widget代表的是不同的部件，例如开关（switch）、按钮（btn）、图片（img），等等。一般情况下，创建对象的函数只有一个形参，那就是*parent，它指向父类，该父类可以是当前的活动屏幕（lv_screen_active）或者是其他的部件。
...
用户需要获取当前活动的屏幕，可以调用lv_screen_active函数，其将返回指向活动屏幕的指针。在部件的创建中，我们经常会获取当前活动的屏幕，将其作为父类。
...
表 6.3.3.2 外部对齐模式（对象之间无父子关系）  

| 常量                        | 说明               |
|-----------------------------|--------------------|
| `LV_ALIGN_OUT_TOP_LEFT`     | 在目标顶部左侧外对齐 |
| `LV_ALIGN_OUT_TOP_MID`      | 在目标顶部正外对齐   |
| `LV_ALIGN_OUT_TOP_RIGHT`    | 在目标顶部右侧外对齐 |
| `LV_ALIGN_OUT_BOTTOM_LEFT`  | 在目标底部左侧外对齐 |
| `LV_ALIGN_OUT_BOTTOM_MID`   | 在目标底部正外对齐   |
| `LV_ALIGN_OUT_BOTTOM_RIGHT` | 在目标底部右侧外对齐 |
| `LV_ALIGN_OUT_LEFT_MID`     | 在目标左侧正外对齐   |
| `LV_ALIGN_OUT_RIGHT_MID`    | 在目标右侧正外对齐   |
...
```c
lv_obj_t \* obj2  $=$  lv_obj_create(lv_screen_active()); /\*创建一个对象\*/lv_obj_add_style(obj2，&style_btn，LV_STATE_DEFAULT); /\*添加对象2的样式\*/
```
...
由上述源码可知，本地样式的设置非常简单，只需要直接将样式设置到某个部件上即可，注意：在上述的lv_obj_set_style_bg_color函数中，第三个形参代表“状态及组成部分选择器”，用户可以在此指定所设置的样式应用到哪一个组成部分，同时也可选择该样式在什么状态下生效。接下来，我们将详细介绍部件的组成部分以及状态，这对于正确地设置样式非常重要。
...
```txt
/* 添加/清除选中状态 */
lv_obj_add_state / lv_obj_remove_state(obj, part, LV_STATE_CHECKED);
```
...
/\*设置按钮的背景颜色，按下时才生效\*/  
lv_obj_set_style_bg_color(btn，lv_color_hex(0xff0000)，LV_STATElongleftrightarrow);
```
...
②lv_style_set_alignLv_obj，LV_ALIGN_CENTER)；
```
...
LV_ALIGN BOTTOM LEFT/MID/RIGHT.  
...
设置背景图像源，该图像源可从lv_image_dsc_t的指针（C语言数组）、文件路径（例如外部SD卡）或内部的字体图标中获取，示例如下：
...
常见文本装饰：`LV_TEXT_DECOR_NONE`（无装饰）、`LV_TEXT_DECOR_UNDERLINE`（下划线）、`LV_TEXT_DECOR_STRIKETHROUGH`（删除线）。

# 6.(text_align) 文本对齐属性

对象文本对齐方式：`LV_TEXT_ALIGN_LEFT`、`LV_TEXT_ALIGN_CENTER`、`LV_TEXT_ALIGN_RIGHT`、`LV_TEXT_ALIGN_AUTO`。
...
任何 LVGL 对象都支持滚动。当子内容超出父对象范围时，父对象会变为可滚动，并根据样式显示滚动条。
...
| `LV_SCROLLBAR_MODE_ACTIVE`| 滚动过程中显示，停止后隐藏     |
| `LV_SCROLLBAR_MODE_AUTO`  | 仅在内容可滚动时显示           |
...
`dir` 可组合使用：`LV_DIR_HOR`、`LV_DIR_VER`、`LV_DIR_LEFT`、`LV_DIR_RIGHT`、`LV_DIR_TOP`、`LV_DIR_BOTTOM`、`LV_DIR_ALL`。

## 6.5.3 其他特性

- **滚动传递**：`LV_OBJ_FLAG_SCROLL_CHAIN_HOR/VER`。子对象滚动到边缘后可将多余滚动传递给父对象。
- **滚动惯性**：`LV_OBJ_FLAG_SCROLL_MOMENTUM`。模拟甩动后的惯性滚动。
- **弹性滚动**：`LV_OBJ_FLAG_SCROLL_ELASTIC`。内容超出范围后回弹至合法区域。
# 6.6 LVGL 动画

动画是 LVGL 中非常重要的动态效果工具。动画本质上是在一定时间内把一个数值从起始值过渡到结束值，并在回调中利用这个数值去更新对象的属性。
...
lv_anim_set_values(&anim, start, end);          /* 起始/结束值 */
lv_anim_set_time(&anim, 500);                   /* 持续时间（毫秒） */
lv_anim_set_exec_cb(&anim, anim_cb);            /* 回调函数 */

lv_anim_start(&anim);
```

`anim_cb` 负责在动画进行过程中更新对象属性：

```c
static void anim_cb(void * var, int32_t v)
{
    lv_obj_set_x(var, v);                        /* 示例：更新 X 坐标 */
}
```
...
```c
lv_anim_set_delay(&anim, 200);                   /* 延迟 200 ms 启动 */
lv_anim_set_repeat_count(&anim, LV_ANIM_REPEAT_INFINITE);
lv_anim_set_playback_delay(&anim, 100);          /* 回放前延迟 */
lv_anim_set_playback_time(&anim, 300);           /* 倒放时长 */
```
...
常见缓动：`lv_anim_path_linear`、`lv_anim_path_step`、`lv_anim_path_ease_in/out`、`lv_anim_path_overshoot`、`lv_anim_path_bounce` 等。
...
void lv_example_anim_move(void)
{
    lv_obj_t * label = lv_label_create(lv_screen_active());
    lv_label_set_text(label, "ALIENTEK");
...
lv_anim_timeline_add(timeline, 0, &anim_w);
lv_anim_timeline_add(timeline, 300, &anim_h);
...
| 函数                        | 说明                         |
|-----------------------------|------------------------------|
| `lv_anim_init`              | 初始化动画描述结构体         |
| `lv_anim_set_var`           | 设置目标对象                 |
| `lv_anim_set_values`        | 设置动画起止值               |
| `lv_anim_set_time`          | 设置动画持续时间             |
| `lv_anim_set_delay`         | 设置启动延迟                 |
| `lv_anim_set_path_cb`       | 设置动画路径（缓动）         |
| `lv_anim_set_exec_cb`       | 设置执行回调                 |
| `lv_anim_set_repeat_count`  | 设置重复次数                 |
| `lv_anim_set_playback_time` | 设置倒放时间                 |
| `lv_anim_start`             | 启动动画                     |
# 6.7 LVGL 定时器
...
用户可通过lv_timer_get_idle函数获得lv_timer_handler（定时器任务）的空闲时间百分比，以此测量出LVGL的空闲时间。注意：该函数仅测量lv_timer_handler的空闲时间，而不会测量整个系统的空闲时间，因此，当用户使用操作系统并在定时器中调用lv_timer_handler时，可能会产生误差，因为它不会实际测量操作系统在空闲线程中花费的时间。
...
```c
lv_result_t lv_event_send(lv_obj_t * obj,  
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td colspan="2">操作文件API</td></tr><tr><td>lv.fs_open()</td><td>打开一个文件</td></tr><tr><td>lv.fs_close()</td><td>关闭一个文件</td></tr><tr><td>lv_fs_read()</td><td>读取文件内容</td></tr><tr><td>lv.fs_write()</td><td>写入数据到文件中</td></tr><tr><td>lv.fs_seek()</td><td>设置&#x27;cursor&#x27;(读/写指针)在文件中的位置</td></tr><tr><td>lv.fs_tell()</td><td>获取读写指针的位置</td></tr><tr><td colspan="2">操作文件夹API</td></tr><tr><td>lv.fs_dir_open()</td><td>打开一个目录</td></tr><tr><td>lv.fs_dir_read()</td><td>读取下一个文件名形成一个目录</td></tr><tr><td>lv.fs_dir_close()</td><td>关闭目录读取</td></tr></table>
...
表 7.2.1.6 lv_fs-seeking 函数形参描述  
...
\*/ long lv.tell(lv.fs_file_t *fd) { uint32_t pos  $= 0$  . lv.fs.tell(fd, &pos); return pos; } /\*\* \* @brief 文件系统测试 \* @param 无 \* @return 无 \*/ static void lv.fs_test(void) { char rbuf[30]  $= \{0\}$  . uint32_t rsize  $= 0$  . lv.fs_file_t fd; lv.fs_res_t res; res  $=$  lv.fs_open(&fd, "0:/SYSTEM/LV_FATFS/Fatfs_test.txt", LV_FS_MODE_RD); if (res != LV_FS_RES_OK) { printf("open 0:/Fatfs_test.txt ERROR\n"); return ; } lv.tell(&fd); lv.fs-seeking(&fd, 0,LV_FS_SEEK_SET); lv.tell(&fd); res  $=$  lv_fs_read(&fd, rbuf, 100, &rsize);
```

```c
if(res != LV_FS_RES_OK) { printf("read ERROR\n"); return; } lv_tell(&fd); printf("READ(%d): %s",rsize, rbuf); lv_fs_close(&fd); } /\*\* \* @brief 文件系统演示 \* @param 无 \* @return 无 \*/ void lv_mainstart(void) { lv_obj_t \*label  $=$  lv_label_create(lv_screen_active()); lv_label_set_text(label, "FATFS TEST"); lv_obj_center.label); lv.fs_test(); }
```

上述源码可分为两个部分：

(1) 标签显示, 此部分用于显示实验名称: FATFS TEST;  
② 文件系统测试，我们首先调用 lv.fs_open 函数打开文件，文件正常打开之后，调用 lv_tell（lv.fs_tell）函数，获取指针的位置，然后再调用 lv_fs_read 函数，读取文件内容，文件内容读取成功后，将其打印到串口，最后调用 lv.fs_close 函数关闭文件。
...
LVGL支持2种编码方式：第一种是ASCII编码，这种编码只支持英文字符的显示；第二种是UTF-8编码，这种编码可以支持全球所有字符的显示。用户需要在LVGL工程中启用UTF-8编码，可以打开lv_conf.h文件，修改LV_TXT_ENC配置项，如下源码所示：
...
{ lv_obj_t \*label  $=$  lv_label_createlv_screen_active(); lv_label_set_text(label，LV_SYMBOL=AUDIO"AUDIO");
```
...
```lisp
void lv_mainstart(void)   
{ lv_obj_t\* label  $=$  lv_label_createlv_screen_active(); lv_obj_set_style_text_Font.label,&lv_font_montserrat_16,LV_STATE_DEFAULT); lv_label_set_text(label,"Hello ALIENTEK！！！"）;   
}
```

在上述代码中，调用了lv_obj_set_style_text_Font函数，并在该函数中设置了16号字体（lv.Font_montserrat_16），如果大家需要设置其他的内部字体，只需要修改一下后缀即可，例如：18号字体为lv.Font_montserrat_18，以此类推。值得注意的是，字号越大，文字越大，但其占用的内存也越多。示例代码的效果如下图所示：
...
void lv_mainstart(void)   
{ lv_obj_t \*font_label  $=$  lv_label_createlv_screen_active(); lv_obj_set_style_text_Font(font_label，&my_Font14，LV_STATE_DEFAULT); lv_label_set_text(font_label，"你好"); lv_obj_center(font_label);   
}
```
...
```txt
void lv_mainstart(void)   
{ lv_obj_t\* label  $=$  lv_label_createlv_screen_active(); lv_obj_set_style_text_Font.label,&Font12,LV_STATE_DEFAULT); lv_label_set_text(label,"Hello ALIENTEK!!!!");   
}
```
...
```python
void lv_obj_set_ext.click_area lv_obj_t * obj, lv_coordinate_t size)
```
...
{  
/* 动态获取屏幕大小 */  
scr_act_width = lv_obj_get_width(lv_screen_active());  
scr_act_height = lv_obj_get_height(lv_screen_active());  
/* 父对象 */  
parent_obj = lv_obj_create(lv_screen_active()); /* 创建父对象 */  
/* 设置父对象的大小 */  
...
```c
void lv_mainstart(void)  
{ /*定义并创建色环*/ lv_obj_t* colorwheel = lv_colorwheel_create lv_screen_active(), true); lv_obj_set_size(colorwheel, 100, 100); lv_obj_center(colorwheel); /*设置色环位置*/
```
...
/* 定义并创建色环 */
lv_obj_t* colorwheel1 = lv_colorwheel_create lv_screen_active(), true);
...
/* 定义并创建色环 */
lv_obj_t* colorwheel2 = lv_colorwheel_create (lv_screen_active(), true);
...
{/\* 创建色环（用于选择颜色）\*/lv_obj_t *colorwheel  $=$  lv_colorwheel_create(lv_screen_active(), true);lv_obj_set_size(colorwheel, scr_act_height()  $\star$  2 /3,scr_act_height()  $\star$  2 /3); /\*设置大小\*/lv_obj_center(colorwheel); /\*设置位置\*/lv_obj_set_style.arc_width(colorwheel, scr_act_height(  $\star$  0.1,LV_PART_MAIN); /\*设置色环圆弧宽度\*/lv_colorwheel_set_mode_fixcd(colorwheel, true); /\*固定色环模式\*//\* 基础对象（用于显示所选颜色）\*/obj  $=$  lv_obj_create(lv_screen_active()); /\*创建基础对象\*//\*设置大小\*/lv_obj_set_size(obj,scr_act_height() /3,scr_act_height() /3);lv_obj_align_to(obj,colorwheel,LV_ALIGN_CENTER,0,0); /\*设置位置\*/lv_obj_set_style_radius(obj,LV RADIUS_CIRCLE,LV_PART_MAIN);/\*设置圆角\*/lv_obj_set_style_bg_color(obj,lv_colorwheel_get_rgb(colorwheel),LV_PART_MAIN); /\*设置背景颜色\*/lv_obj_add_event_cb(colorwheel,colorwheel_event_cb,LV_EVENT_VALUE_CHANGED,NULL); /\*设置色环事件回调\*/}  
...
void lv_mainstart(void)  
{  
/* 定义并创建图片按钮 */lv_obj_t* imgBTN = lv_imgbtn_create(lv_screen_active());  
/* 设置图片释放时的图片 */lv_imgbtn_set_src(imgBTN, LV_IMGBTN_STATEreleased, NULL, &img_zdyz, NULL);  
/* 设置图片按钮大小 */lv_obj_set_size(imgBTN, 100, 100);  
/* 设置图片按钮位置 */lv_obj_center(imgBTN);lv_obj_set_style_img_recolor_opa(imgBTN, LV_OPA_30, LV_STATElongleftrightarrow);lv_obj_set_style_img_recolor(imgBTN, lv_color_white(), LV_STATElongleftrightarrow);lv_obj_set_style_translate_y(imgBTN, 5, LV_STATElongleftrightarrow);  
}
```
...
如果用户想清除或者添加上述的状态，可调用lv_obj_add_state / lv_obj_remove_state函数进行设置。
...
{lv_obj_t \*target  $=$  lv_event_get_target(e); /\*获取触发源\*/if(cool_mode_state  $= = 0$  /\*判断按钮状态，如果为0，即关闭状态\*/{cool_mode_state  $= 1$  /\*切换按钮状态\*//\*设置图片重新着色透明度\*/lv_obj_set_style_img_recolor_opa(target，255，0);/\*设置图片重新着色：蓝色\*/lv_obj_set_style_img_recolor(target，lv_color_hex(0x00a9ff)，0);1else/\*按钮为开启状态\*/{cool_mode_state  $= 0$  /\*切换按钮状态\*//\*设置图片重新着色透明度\*/lv_obj_set_style_img_recolor_opa(target，255，0);/\*设置图片重新着色：灰色\*/lv_obj_set_style_img_recolor(target，lv_color_hex(0x8a8a8a)，0);1  
}  
/\*  
...
```txt
{lv_obj_t \*target  $=$  lv_event_get_target(e); /\*获取触发源\*/if(warm_mode_state  $\equiv 0$  /\*判断按钮状态，如果为0，即关闭状态\*/{warm_mode_state  $= 1$  /\*切换按钮状态\*//\*设置图片重新着色透明度\*/lv_obj_set_style_img_recolor_opa(target,255,0);/\*设置图片重新着色：红色\*/lv_obj_set_style_img_recolor(target,lv_color_hex(0xff0000),0);1}else{warm_mode_state  $= 0$  /\*切换按钮状态\*//\*设置图片重新着色透明度\*/lv_obj_set_style_img_recolor_opa(target,255,0);\*\*设置图片重新着色：灰色\*/lv_obj_set_style_img_recolor(target,lv_color_hex(0x8a8a8a),0);1}  
\*/\*\*@brief干燥按钮事件回调\* @param \*e：事件相关参数的集合，它包含了该事件的所有数据\* @return无\*/static void dry_event_cb(lv_event_t \*e)  
{lv_obj_t \*target  $=$  lv_event_get_target(e); /\*获取触发源\*/if(dry_mode_state  $\equiv 0$  /\*判断按钮状态，如果为0，即关闭状态\*/{dry_mode_state  $= 1$  /\*切换按钮状态\*//\*设置图片重新着色透明度\*/lv_obj_set_style_img_recolor_opa(target,255,0);\*\*设置图片重新着色：蓝色\*/lv_obj_set_style_img_recolor(target,lv_color.hex(0x00a9ff)，0);1}else/\*按钮为开启状态\*/{dry_mode_state  $= 0$  /\*切换按钮状态\*//\*设置图片重新着色透明度\*/lv_obj_set_style_img_recolor_opa(target,255,0);
```
...
{/\*根据活动屏幕宽度选择字体\*/if (scr_act_width() <= 480){font  $=$  &lv.Font_montserrat_14;1else{font  $=$  &lv.Font_montserrat_22;1/\*基础对象（用作背景）\*/lv_obj_t \*obj  $=$  lv_obj_create(lv_screen_active());/\*创建基础对象\*/lv_obj_set_size(obj,scr_act_width(-10,200);\*/设置大小\*/lv_obj_align(obj,LV_ALIGN_CENTER,0,0);/\*设置位置\*//\*制冷模式（图片按钮）\*/lv_obj_t \*imgbtn_cool  $=$  lv_imgbtn_create(obj);\*/创建图片按钮\*/lv_imgbtn_set_src(imgbtn_cool，LV_IMGBTN_STATE_RELEASED，NULL,&img_cool，NULL);/\*设置图片源\*/lv_obj_set_size(imgbtn_cool,64,64);/\*设置大小\*//\*设置位置\*/lv_obj_align(imgbtn_cool，LV_ALIGN CENTER，-scr act width(/3,-15);\*/添加事件\*/lv_obj_add_event_cb(imgbtn_cool，cool_event_cb，LV_EVENTlongleftrightarrow，NULL);/\*制冷模式（标签）\*/lv_obj_t \*label_cool  $=$  lv_label_create(obj);/\*创建标签\*/lv_label_set_text(labelcale,"Cool");/\*设置文本\*/lv_obj_set_style_text_Font label�cool，font，0);/\*设置字体\*//\*设置位置\*/lv_obj_align_to labelled_cool，imgbtn_cool，LV_ALIGN_OUT BOTTOM_MID，0，10);
```
...
/* 设置位置 */
lv_obj_align_to(label_warm, imgBTN_warm, LV_ALIGN_OUT BOTTOM_MID, 0, 10);
```
...
/* 设置位置 */
lv_obj_align_to(label_dry, imgBTN_dry, LV_ALIGN_OUT BOTTOM_MID, 0, 10);
```
...
/* 设置位置 */
lv_obj_align(line_left, LV_ALIGN_CENTER, -scract_width(/6, 0));
```

```c
lv_line_set_points(line_right，linepoints，2); /\*设置线条坐标点\*/lv_obj_set_style_line_color(line_right，lv_color_hex(0xc6c6c6)，LV_STATE_DEFAULT); /\*设置线条颜色\*//\*设置位置\*/lv_obj_align(line_right，LV_ALIGN_CENTER，scr_act_width(/6,0);1/****** 第二部分 结束 \*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*
```
...
```c
define scr_act_width() lv_obj_get_width(lv_screen_active())
#define scr_act_heightlv_obj_get_heightLv_scract()
void lv_mainstart(void)
\*/定义并创建键盘 \*/lv_obj_t* keyboard  $=$  lv_keyboard_create(lv_screen_active());/\*定义并创建文本框\*/
```

```c
lv_obj_t* texture area = lv_textarea_create lv_screen_active();  
/* 设置文本框宽度 */  
...
static void lv_example_led_1(void)
{
    ...
    lv_obj_set_style_bg_color(obj, lv_color_hex(0xe9efef), LV_STATE_DEFAULT);
...
```c
lv_obj_t *label = lv_label_create(lv_screen_active()); /* 创建LED功能标签*/ lv_label_set_text(label, "ROOM 1"); /* 设置文本*/ lv_obj_set_style_text_Font (label,font,LV_STATE_DEFAULT); /* 设置字体*/ lv_obj_align_to labelled,led,LV_ALIGN_OUT BOTTOM_MID,0, scr_act_height(/15); /*设置位置*/   
}   
/\*\* \* @brief LED2 \* @param 无 \* @return 无   
\*/   
static void lv_example_led_2(void)   
{ lv_obj_t\* led  $=$  lv_led_create(obj); /*创建LED*/ /\*设置LED大小\*/ lv_obj_set_size(led,scr_act_height() /5,scr_act_height() /5); /\*设置LED位置\*/ lv_obj_align(led,LV_ALIGN CENTER,0,-scr_act_height() /15); lv_led_set_color(led,lv_color.hex(0xff0000)); /*设置LED颜色*/ lv_led_on(led); /\*打开LED\*/ /\*设置LED事件回调\*/ lv_obj_add_event_cb(led,led_event_cb,LV_EVENTCLICKED，NULL); lv_obj_t\*label  $=$  lv_label_createLv_scr_actu(); /\*创建LED功能标签\*/ lv_label_set_text(label,"ROOM 2"); /\*设置文本\*/ lv_obj_set_style_text_font (label,font,LV_STATE_DEFAULT); /\*设置字体\*/ lv_obj_align_to labelled,led,LV_ALIGN_OUT BOTTOM_MID,0, scr_act_height() /15); /\*设置位置\*/   
}   
/\*\* \* @brief LED3 \* @param 无 \* @return 无   
\*/   
static void lv_example_led_3(void)   
{ lv_obj_t\* led  $=$  lv_led_create(obj); /\*创建LED\*/ /\*设置LED大小\*/ lv_obj_set_size(led,scr_act_height() /5,scr_act_height() /5); lv_obj_align(led,LV_ALIGN CENTER,scr_act_width()  $\star$  4/15, -scr_act_height() /15); /\*设置LED位置\*/
```
...
/* 设置LED事件回调 */
lv_obj_add_event_cb(led, led_event_cb, LV_EVENTCLICKED, NULL);
lv_obj_t *label = lv_label_create(lv_screen_active()); /* 创建LED功能标签 */
lv_label_set_text(label, "ROOM3"); /* 设置文本 */
lv_obj_set_style_text_Font.label, font, LV_STATE_DEFAULT); /* 设置字体 */
lv_obj_align_to(label, led, LV_ALIGN_OUT BOTTOM_MID, 0, scr_ACT_height(); /* 设置位置 */
********** 第二部分 结束 *** /
```
...
```c
void lv_mainstart(void)  
{lv_obj_t *list = lv_list_create(lv_screen_active()); /\* 创建列表 \*/lv_obj_set_width(list, 200); /\* 设置列表宽度 \*/lv_obj_set_height(list, 150); /\* 设置列表高度 \*/lv_obj_center(list);lv_obj_t \*BTN;BTN = lv_list_addBTN(list, LV_SYMBOL_FILE, "New"); /\* 添加按钮 \*/BTN = lv_list_addBTN(list, LV_SYMBOL_DIRECTORY, "Open"); /\* 添加按钮 \*/}
```
...
```c
void lv_mainstart(void)  
{lv_obj_t* list = lv_list_create lv_screen_active(); /\* 创建列表 \*/lv_obj_set_width(list, 200); /\* 设置列表宽度 \*/lv_obj_set_height(list, 150); /\* 设置列表高度 \*/lv_obj_center(list);lv_obj_t\* btn;lv_list_add_text(list, "File"); /\* 列表添加标签 \*/btn = lv_list_add_btn(list, LV_SYMBOL_FILE, "New"); /\* 添加按钮 \*/btn = lv_list_add_btn(list, LV_SYMBOL_DIRECTORY, "Open"); /\* 添加按钮 \*/}
```
...
/* 创建左侧矩形背景 */  
```c
lv_obj_t* obj_left = lv_obj_create(lv_screen_active()); /* 创建一个基础对象 */
...
/* 创建右侧矩形背景 */  
```c
lv_obj_t* obj_right = lv_obj_create(lv_screen_active()); /* 创建一个基础对象 */
...
<table><tr><td>lv_meter_set_scale;ticks()</td><td>设置小刻度</td></tr><tr><td>lv_meter_set_scalemajor;ticks()</td><td>设置主刻度</td></tr></table>
...
# (2)lv_meter_set_scale_tickss函数

给仪表添加一个小刻度，该函数原型如下所示：

```c
void lv_meter_set_scale_tickss(lv_obj_t *obj, lv_meter_scale_t *scale, uint16_t cnt, uint16_t width, uint16_t len, lv_color_t color);
```

该函数的形参，如下表所示：

表 32.3.1.3 lv_meter_set_scale_tickss 函数形参描述  
...
/* 获取当前活动屏幕的宽高 */
#define scr_act_width() lv_obj_get_width(lv_screen_active())
#define scr_act_height() lv_obj_get_height(lv_screen_active())
void lv_mainstart(void)
{
    ...
    lv_meter_set_scale_tickss(metric, scale, 41, 1, scr_act_height() / 80,
                    lv_palette_main(LV_PALETTE_GREY));
    /* 设置主刻度的步长为8，宽度为1，长度为屏幕高度除以60，颜色为黑色，
刻度与数值的间距为屏幕高度除以30 */
    lv_meter_set_scale_major_ticks(metric, scale, 8, 1, scr_act_height() / 60,
                    lv_color_white(), scr_act_height() / 30);
}
```
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td>lv_meter_add_needle_line()</td><td>添加指针</td></tr><tr><td>lv_meter_set_indicator_value()</td><td>设置指针指向的数值</td></tr></table>
...
/* 获取当前活动屏幕的宽高 */
#define scr_act_width() lv_obj_get_width(lv_screen_active())
#define scr_act_height() lv_obj_get_heightlv_screen_active()
void lv_mainstart(void)
```

```javascript
{lv_obj_t\* meter  $=$  lv_meter_create(lv_screen_active());/\*定义并创建仪表\*/lv_obj_set_width(meter,scr_act_height(  $\text{串}$  0.4）；/\*设置仪表宽度\*/lv_obj_set_height(meter,scr_act_height(  $\text{串}$  0.4）；/\*设置仪表高度\*//\*设置仪表位置\*/lv_obj_center(meter);/\*设置仪表刻度\*/lv_meter_scale_t\* scale  $=$  lv_meter_add_scale(meter);/\*定义并添加刻度\*//\*设置小刻度\*/lv_meter_set_scale_tickss(meter, scale,41,1,scr_act_height() /80，lv_palette_main(LV_PALETTEGREY));/\*设置主刻度\*/lv_meter_set_scalemajor_ticks(meter, scale,8,1,scr_act_height() /60，lv_color_white(),scr_act_height() /30);lv_meter(indicator_t\*indic;/\*添加仪表指针，该指针宽度为4，颜色为灰色，长度-10\*/indic  $=$  lv_meter_add_needle_line(meter, scale,4，lv_palette_main(LV_PALETTEGREY)，-10);/\*设置指针指向的数值\*/lv_meter_set_indicator_value(meter,indic,54);
```

在上述源码中，我们调用了lv_meter_add_needle_line函数来添加仪表指针，该指针的宽度为4，颜色为灰色，指针长度为-10，添加完指针之后，该函数会返回仪表指针对象，有了指针对象，再将仪表指针的数值设置为54。上述源码可在PC模拟器或者在开发板上运行，其程序效果图如下图所示：

![](images/8662ba9c65e07e9d23491be20ffb816aba25f10305cbb381f17ef1f7de2d1050.jpg)  
图32.2.1.2添加指针并设置指向的数值

上图中，仪表指针的顶端和小刻度还有一定的距离，该距离由lv_meter_add_needle_line函数最后的形参决定，如果传入的数值为0，则仪表指针刚好覆盖到小刻度的顶端；如果传入的数值大于0，则仪表指针超出小刻度；如果传入的数值小于0，则仪表指针的顶端和小刻度还有一定的距离。
...
{
    lv_obj_t* meter = lv_meter_create(lv_screen_active());
    lv_obj_set_width(meter, scr_act_height() * 0.4); /* 定义并创建仪表 */
    lv_obj_set_height(meter, scr_act_height() * 0.4); /* 设置仪表宽度 */
    /* 设置仪表位置 */
    lv_obj_center(meter);
    /* 设置仪表刻度 */
    lv_meter_scale_t* scale = lv_meter_add_scale(meter); /* 定义并添加刻度 */
    /* 设置小刻度 */
    lv_meter_set_scale_tickss(meter, scale, 41, 1, scr_act_height() / 80,
                    lv_palette_main(LV_PALETTEGREY));
    /* 设置主刻度 */
};
```

```c
lv_meter_set_scalemajor_ticks(meter, scale, 8, 1, scr_act_height() / 60, lv_color/black(), scr_act_height() / 30);  
lv_meter_indicator_t* indic;  
/* 添加仪表指针，该指针宽度为4，颜色为灰色，长度-10*/  
indic = lv_meter_add_needle_line(meter, scale, 4, lv_palette_main(LV_PALETTE_GREY), -10);  
/* 设置指针指向的数值 */  
lv_meter_setindicator_value(meter, indic, 54);  
/* 设置仪表数值范围、有效角度和旋转角度 */  
...
void lv_mainstart(void)
{
    ...
    lv_meter_scale_t* scale = lv_meter_add_scale(meter); /* 定义并添加刻度 */
    /* 设置小刻度 */
    lv_meter_set_scale_tickss(meter, scale, 41, 1, scr_act_height() / 80, lv_palette_main(LV_PALETTE_GREY));
    /* 设置主刻度 */
    lv_meter_set_scalemajor_ticks(meter, scale, 8, 1, scr_act_height() / 60, lv_color_white(), scr_act_height() / 30);
    /* 添加指针图像 */
    lv_meter_indicator_t * indic = lv_meter_add_needle_img(metric, scale, &img_hand, 5, 5);
    /* 设置指针指向的数值 */
    lv_meter_setIndicator_value(meter, indic, 54);
}
```
...
注意：lv_meter_add_needle.img函数的最后两个形参需要根据实际情况进行设置，如果图片和原点有偏差，大家可以适当修改这两个形参的数值。

# 2. 仪表的指示刻度

仪表的指示刻度是指在仪表刻度上指定某个范围刻度填充颜色，前面我们学习过lv_meter_set_scale_tickss和lv_meter_set_scale_major_ticks这两个函数，它们都有一个形参表示刻度填充的颜色，显然地，这些形参只针对整体设置颜色，并不是指定某个刻度范围设置颜色。这个次要功能也是比较常见的，比如汽车油表，它的最低值和最大值的某范围都是使用明显的颜色提示和引导驾驶员。下面我们使用一个示意图来讲解这个功能，如下图所示：
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td>lv_meter_add_scale-lines()</td><td>添加刻度线指示刻度</td></tr><tr><td>lv_meter_set_indicator_start_value()</td><td>设置指示刻度的起始值</td></tr><tr><td>lv_meter_set Indicator_end_value()</td><td>设置指示刻度的终止值</td></tr></table>

上述表中的函数也是按照某些顺序执行，首先我们调用函数lv_meter_add_scale-lines在仪表上添加指示刻度，该函数返回指示刻度指针，然后程序根据指示刻度指针设置指示刻度的起始值和终止值，起始值和终止值无需顺序调用。注意：如果仪表不设置起始值，则指示刻度从0开始；如果仪表只添加指示刻度不设置起始值和终止值，则指示刻度不会被绘画出来。
...
```c
lv_meter_indicator_t *lv_meter_add_scale-lines(lv_obj_t *obj,  
lv_meter_scale_t *scale,  
...
表 32.2.2.3 lv_meter add scale lines 函数形参描述
...
# (3)lv_meter_setindicator_end_value函数
...
表 32.2.2.4 lv_meter_setindicator_end_value 函数形参描述
...
```c
void lv_mainstart(void)   
{ lv_obj_t\* meter  $=$  lv_meter_create(lv_screen_active(); /\*定义并创建仪表\*/ lv_obj_set_width(meter,scr_act_height(); /\*设置仪表宽度\*/ lv_obj_set_height(meter,scr_act_height(); /\*设置仪表高度\*/ /\*设置仪表位置\*/ lv_obj_center(meter); /\*设置仪表刻度\*/ lv_meter_scale_t\* scale  $\equiv$  lv_meter_add_scale(meter); /\*定义并添加刻度\*/ /\*设置小刻度\*/ lv_meter_set_scale_tickss(meter, scale,41,1,10, lv_palette_main(LV_PALETTEGREY)); /\*设置主刻度\*/ lv_meter_set_scalemajor_ticks(meter, scale,8,1,10, lv_color_white(),scract_height() /30); /\*添加指针图像\*/ lv_meter_indicator_t\* indic  $\equiv$  lv_meter_add_needle_img(metric, scale, &img_hand,5,5); /\*设置指针指向的数值\*/ lv_meter_set_indicator_value(meter,indic,54); /\*添加指示刻度，起始颜色为浅蓝色，终止颜色为深蓝色，状态为true，指示刻度为10\*/ indic  $=$  lv_meter_add_scale Lines(meter, scale, lv_palette DARKEN(LV_PALETTE_BLUE,1), lv_palette DARKen(LV_PALETTE_BLUE,4), true,10); /\*设置指示刻度起始值为50\*/ lv_meter_set_indicator_start_value(meter,indic,50); /\*设置指示刻度终止值为100\*/ lv_meter_set_indicator_end_value(meter,indic,100); }
```
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td>lv_meter_addArc()</td><td>添加弧线指示器</td></tr><tr><td>lv_meter_setindicator_start_value()</td><td>设置指示刻度的起始值</td></tr><tr><td>lv_meter_setindicator_end_value()</td><td>设置指示刻度的终止值</td></tr></table>
...
```txt
lv_meter_indicator_t *lv_meter_add.arc(lv_obj_t *obj,
```
...
void lv_mainstart(void)
{
    lv_obj_t* meter = lv_meter_create(lv_screen_active()); /* 定义并创建仪表 */
    ...
    lv_meter_scale_t* scale = lv_meter_add_scale(meter); /* 定义并添加刻度 */
    /* 设置小刻度 */
    lv_meter_set_scale_tickss(meter, scale, 41, 1, 10,
                          lv_palette_main(LV_PALETTEGREY));
    /* 设置主刻度 */
    lv_meter_set_scalemajor_ticks(meter, scale, 8, 1, 10,
                          lv_color_white(), scr_act_height() / 30);
    /* 添加指针图像 */
    lv_meter(indicator_t * indic = lv_meter_add_needle.img(meter,
                          scale,
                                  &img_hand, 5, 5);
    /* 设置指针指向的数值 */
    lv_meter_set_indicator_value(meter, indic, 54);
    /* 添加指示刻度，起始颜色为浅蓝色，终止颜色为深蓝色，状态为 true，指示刻度为 10 */
    indic = lv_meter_add_scale-lines(meter, scale,
                          lv_palette DARKEN(LV_PALETTE_BLUE, 1),
                          lv_palette DARKEN(LV_PALETTE_BLUE, 4),
                          true, 10);
```

```c
/\*设置指示刻度起始值为50\*/  
lv_meter_setindicator_start_value(metric，indic，50);  
/\*设置指示刻度终止值为100\*/  
lv_meter_setindicator_end_value(metric，indic，100);  
/\*添加弧线指示器，弧线宽度为10，颜色为红色，偏移10\*/  
indic  $=$  lv_meter_add_arc(metric，scale，10,lv_palette_main(LV_PALETTE_red)，10);  
lv_meter_setindicator_start_value(metric，indic，50);  
lv_meter_setindicator_end_value(metric，indic，100);
```

在上述源码中，我们调用了lv_meter_addArc函数来添加弧线指示器，该弧线指示器的宽度为10，弧线颜色为红色，弧线偏移10。示例代码可在PC模拟器上运行，效果如下所示：
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td>lv_meter_create()</td><td>创建仪表对象</td></tr><tr><td>lv_meter_add_scale()</td><td>给仪表添加新刻度</td></tr><tr><td>lv_meter_set_scale_tickss()</td><td>设置小刻度的属性</td></tr><tr><td>lv_meter_set_scalemajor_ticks()</td><td>设置主刻度的属性</td></tr><tr><td>lv_meter_set_scale_range()</td><td>设置数值范围和角度范围</td></tr><tr><td>lv_meter_add_needle_line()</td><td>添加指针</td></tr></table>

表 32.3.1 仪表部件相关的 API 函数  

<table><tr><td>lv_meter_add_needle_img()</td><td>添加图片指针</td></tr><tr><td>lv_meter_addArc()</td><td>添加弧线指示器</td></tr><tr><td>lv_meter_add_scale-lines()</td><td>添加刻度线指示器</td></tr><tr><td>lv_meter_set_indicator_value()</td><td>设置指针的值</td></tr><tr><td>lv_meter_set_indicator_start_value()</td><td>设置刻度的起始值</td></tr><tr><td>lv_meter_set_indicator_end_value()</td><td>设置刻度的终止值</td></tr></table>
...
{const lv_font_t* font;/\*根据屏幕大小设置字体\*/if (scr_act_width() <= 320)font  $=$  &lv.Font_montserrat_10;else if (scr_act_width() <= 480)font  $=$  &lv.Font_montserrat_14;elsefont  $=$  &lv.Font_montserrat_16;/\*定义并创建仪表\*/lv_obj_t\* meter  $\equiv$  lv_meter_create(lv_screen_active());/\*设置仪表宽度\*/lv_obj_set_width(meter,scr_act_height()  $\ast$  0.4);/\*设置仪表高度\*/lv_obj_set_height(meter,scr_act_height()  $\ast$  0.4);/\*设置仪表位置\*/lv_obj_align(metric,LV_ALIGN_CENTER,-scr_act_width() /3,0);\*/设置仪表文字字体\*/lv_obj_set_style_text_Font(metric,font,LV_PART_MAIN);/\*设置仪表刻度\*//\*定义并添加刻度\*/lv_meter_scale_t\* scale  $\equiv$  lv_meter_add_scale(metric);\*/设置小刻度\*/lv_meter_set_scale_tickss(metric,scale,41,1,scr_act_height() /80,lv_palette_main(LV_PALETTEGREY));/\*设置主刻度\*/lv_meter_set_scalemajor_ticks(metric,scale,8,1,
```

```txt
$\mathrm{scr\_act\_height() / 60,lv\_color\_black(),}$  scr_act_height() /30); /\*设置指针\*/ lv_meterindicator_t\*indic; indic  $=$  lv_meter_addArc(meter, scale,2, lv_palette_main(LV_PALETTE_BLUE),0); lv_meter_setindicator_start_value(meter,indic,0); lv_meter_setindicator_end_value(meter,indic,20); indic  $=$  lv_meter_add_scale-lines(meter, scale, lv_palette_main(LV_PALETTE_BLUE), lv_palette_main(LV_PALETTE_BLUE),false,0); lv_meter_setindicator_start_value(meter,indic,0); lv_meter_setindicator_end_value(meter,indic,20); indic  $=$  lv_meter_addArc(meter, scale,2, lv_palette_main(LV_PALETTE RED),0); lv_meter_setindicator_start_value(meter,indic,80); lv_meter_setindicator_end_value(meter,indic,100); indic  $=$  lv_meter_add_scaleLines(meter, scale, lv_palette_main(LV_PALETTE RED), lv_palette_main(LV_PALETTE RED),false,0); lv_meter_setindicator_start_value(meter,indic,80); lv_meter_setindicator_end_value(meter,indic,100); indic  $=$  lv_meter_add_needle_line(meter, scale,4, lv_palette_main(LV_PALETTE GREY)，-10); lv_obj_set_user_data(meter，indic); /\*设置指针动画\*/ lv_anim_t a; lv_anim_init(&a); lvanim_set_exec_cb(&a,(lvanim_exec_xcb_t)anim_1_indic_cb); lvanim_set_var(&a,meter); lvanim_set_values(&a,0，100); lvanim_set_time(&a，2000); lvanim_setrepeat_delay(&a，100); lvanim_set.Playback_time(&a，500); lvanim_set.playback_delay(&a，100); lvanim_setrepeat_count(&a,LV_ANIM_REPEAT_INFINITE); lvanim_start(&a);   
}   
...
{lv_meter_indicator_t\* indic = (lv_meter_indicator_t\*)meter->user_data;lv_meter_setIndicator_value(meter, indic, value);  
}  
/\*\* \* @brief 例2\* @param 无\* @return 无\*/  
static void lv_example_meter_2(void)  
{const lv_font_t\* font;/\* 根据屏幕大小设置字体 \*/if (scr_act_width() <= 320)font  $=$  &lv_font_montserrat_8;else if (scr_act_width() <= 480)font  $=$  &lv_font_montserrat_10;elsefont  $=$  &lv_font_montserrat_14;/\* 定义并创建仪表 \*/meter_2 = lv_meter_create(lv_screen_active());/\* 设置仪表宽度 \*/lv_obj_set_width(meter_2,scr_act_height() \* 0.4);/\* 设置仪表高度 \*/lv_obj_set_height(meter_2,scr_act_height() \* 0.4);/\* 设置仪表位置 \*/lv_obj_center(meter_2);/\* 设置仪表文字字体 \*/lv_obj_set_style_text_Font(meter_2,font,LV_PART_MAIN);/\* 删除仪表指针样式 \*/lv_obj_remove_style(meter_2，NULL，LV_PART_INDICATOR);/\* 设置仪表刻度 \*/lv_meter_scale_t\* scale  $=$  lv_meter_add_scale(meter_2);lv_meter_set_scale_tickss(meter_2, scale,11,2,scract_height() / 15,lv_palette_main(LV_PALETTE_GREY));lv_meter_set_scale_major_ticks(meter_2, scale,1,2,
```

```c
scr_act_height() / 15, lv_color_hex3(0xEEE), scr_act_height() / 40);  
lv_meter_set_scale_range(metric_2, scale, 0, 100, 270, 90);  
/* 设置指针 */  
lv_meter(indicator_t* indic1 = lv_meter_add_arc(metric_2, scale, scr_act_height() / 45, lv_palette_main(LV_PALETTE_RED), 0);  
lv_meter(indicator_t* indic2 = lv_meter_add_arc(metric_2, scale, scr_act_height() / 45, lv_palette_main(LV_PALETTE.Green), -scr_act_height() / 45);  
lv_meter(indicator_t* indic3 = lv_meter_add_arc(metric_2, scale, scr_act_height() / 45, lv_palette_main(LV_PALETTE_BLUE), -scr_act_height() / 22.5);  
/* 设置指针动画 */  
lv_anim_t a;  
lv_anim_init(&a);  
...
```javascript
\* @param value:数值\* @return无\*/static void anim_2_indic_cb(lv_meter_indicator_t\* indic, int32_tvalue)  
{lv_meter_indicator_set_end_value(meter_2，indic，value);  
}  
/\*\* \* @brief 例3\* @param 无\* @return无\*/  
static void lv_example_meter_3(void)  
{/\* 定义并创建仪表\*/lv_obj_t\* meter  $=$  lv_meter_create(lv_screen_active());/\*移除背景样式\*/lv_obj_remove_style(meter，NULL，LV_PART_MAIN);\*/\*移除指针样式\*/lv_obj_remove_style(meter，NULL，LV_PART_INDICATOR);\*/设置仪表宽度\*/lv_obj_set_width(meter，scr(act_height()  $\star$  0.4);\*/设置仪表高度\*/lv_obj_set_height(meter，scr(act_height()  $\star$  0.4);\*/设置仪表位置\*/lv_obj_align(meter，LV_ALIGN_CENTER，scr(act_width() /3,0);\*/手动更新参数\*/lv_obj_update.layout(meter);\*/设置仪表刻度\*/lv_meter_scale_t\* scale  $=$  lv_meter_add_scale(meter);lv_meter_set_scale_tickss(meter，scale，0，0，0，lv_color_white();lv_meter_set_scale_range(meter，scale，0，100，360，0);\*/设置指针\*/lv_coord_t indic_w  $=$  scr_act_height()  $\star$  0.05;lv_meter_indicator_t\* indic1  $=$  lv_meter_add_arc(meter，scale，indic_w,lv_palette_main(LV_PALETTE_ORANGE)，0);lv_meter_indicator_set_start_value(meter，indic1，0);lv_meter_indicator_set_end_value(meter，indic1，40);lv_meter_indicator_t\* indic2  $=$  lv_meter_add_arc(meter，scale，indic_w,lv_palette_main(LV_PALETTE_YELLOW)，0);/\*Start from the previous\*/
```

```c
lv_meter_setindicator_start_value(metric, indic2, 40);  
lv_meter_setindicator_end_value(metric, indic2, 80);  
lv_meter Indicator_t* indic3 = lv_meter_add_arc(metric, scale, indic_w, lv_palette_main(LV_PALETTE_deep_orANGE), 0);  
/*Start from the previous*/  
lv_meter_setindicator_start_value(metric, indic3, 80);  
lv_meter_setindicator_end_value(metric, indic3, 100);
```
...
```c
void lv_mainstart(void)   
{ static const char\* btns[]  $=$  {"Apply", "Close",""}; lv_obj_t\* lv_msgbox  $\equiv$  lvmsgbox_create(NULL, "Hello", "ALIENTEK", btns, true);
```
...
{/\*滑块\*/lv_obj_t \*slider  $=$  lv_slider_createlv_screen_active(); /\*创建滑块\*/lv_obj_set_size(slider,scr_ACT_width() /2,20); /\*设置大小\*/lv_obj_center(slider); /\*设置位置\*/lv_slider_set_value(slider,50,LV_ANIM_OFF); /\*设置当前值\*/lv_obj_add_event_cb(slider,slider_event_cb,LV_EVENT_VALUE_CHANGED，NULL）;/\*百分比标签\*/slider_label  $=$  lv_label_createlv_screen_active(); /\*创建百分比标签\*/lv_label_set_text(slider_label,"50%"); /\*设置文本内容\*/lv_obj_set_style_text_Font(slider_label,&lv_font_montserrat_20, LV_STATE_DEFAULT); /\*设置字体\*/lv_obj_align_to(slider_label,slider,LV_ALIGN_OUT_RIGHT_MID,20,0);
```

```c
/* 音量图标 */
lv_obj_t *sound_label = lv_label_create(lv_screen_active(   )); /* 创建音量标签 */
...
lv_obj_set_style_text_Font(content, &lv-font_montserrat_20, LV_STATE_DEFAULT); /* 设置字体 */  
...
lv_obj_set_style_text_Font(btn, &lv_font_montserrat_20, LV_PART_items); /* 设置文本颜色（未按下）：蓝色 */  
...
/* 设置边框颜色 */
lv_style_set_border_color(&style, lv_palette_main(LV_PALETTE_ORANGE)); lv_style_set_pad_all(&style, 2);
```

第二步：调用lv_spangroup_create函数，创建Span部件组，这个组是用来添加Span部件描述符的，源码如下所示：

```c
lv_obj_t * spans = lv_spangroup_create(lv_screen_active()); /* 创建 spans 组 */  
...
lv_style_set_text_color(&span->style，lv_palette_main(LV_PALETTE_RED));/\*设置该文本的字体\*/  
lv_style_set_text.Font(&span->style，&lv_font_montserrat_24);
```

在上述源码中，我们创建Span描述符并添加到Span组中，然后调用lv_spans_set_text函数，设置要显示的文本，最后调用样式属性设置函数，修改文本的样式。上述的源码可以在模拟器上运行，运行效果如下所示：
...
<table><tr><td>对齐模式</td><td>描述</td></tr><tr><td>LV_TEXT_ALIGN_LEFT</td><td>文本左对齐</td></tr><tr><td>LV_TEXT_ALIGN_CENTER</td><td>文本居中对齐</td></tr><tr><td>LV_TEXT_ALIGN_RIGHT</td><td>文本右对齐</td></tr><tr><td>LV_TEXT_ALIGN_AUTO</td><td>文本自动对齐</td></tr></table>
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td colspan="2">Span部件创建与删除函数</td></tr><tr><td>lv_spangroup_create()</td><td>创建Span组</td></tr><tr><td>lv_spangroup_new spans()</td><td>创建Span描述符</td></tr><tr><td>lv_spangroup_del spans()</td><td>删除Span描述符</td></tr><tr><td colspan="2">Span部件设置函数</td></tr><tr><td>lv_spans_set_text()</td><td>设置文本</td></tr><tr><td>lv_spans_set_text(static)</td><td>设置文本（静态）</td></tr><tr><td>lv_spangroup_set_align()</td><td>设置文本对齐模式</td></tr><tr><td>lv_spangroup_set_overflow()</td><td>设置溢出模式</td></tr><tr><td>lv_spangroup_set_indent()</td><td>设置文本首行缩进</td></tr><tr><td>lv_spangroup_set_mode()</td><td>设置Span组模式</td></tr><tr><td>lv_spangroup_set-lines()</td><td>设置显示行数</td></tr><tr><td colspan="2">Span部件获取函数</td></tr></table>
...
{  
/* 定义并创建 spangroup */lv_obj_t* spangroup = lv_spangroup_create lv_screen_active();  
...
/* 设置 spangroup 边框颜色 */lv_obj_set_style_border_color(spangroup,lv_palette_main(LV_PALETTE_ORANGE),LV_PART_MAIN);  
/* 设置 spangroup 文本对齐方式 */lv_spangroup_set_align(spangroup, LV_TEXT_ALIGN_LEFT);  
...
/* 创建并新建 span */lv_spans_t* span = lv_spangroup_new-span(spangroup);  
/* 设置 span 文本 */lv_spans_set_text span, "This is span 1.";  
/* 设置 span 文本颜色 */lv_style_set_text_color(&span->style, lv_palette_main(LV_PALETTE_RED));  
/* 设置 span 文本格式 */lv_style_set_textdecor(&span->style, LV_TEXT_DECOR_STRIKETHROUGH|LV_TEXT_DECOR_UNDERLINE);
```
...
/* 设置 span 文本颜色 */
lv-style_set_text_color(&span->style, lv_palette_main(LV_PALETTE_GREEN));
/* 新建 span */
span = lv_spangroup_new-span(spangroup);
/* 设置 span 文本 */
lv/style_set_text-static(char* &), "This is span 3. ");
/* 设置 span 文本颜色 */
lv-style_set_text_color(&span->style, lv_palette_main(LV_PALETTE_BLUE));
/* 新建 span */
span = lv_spangroup_new-span(spangroup);
/* 设置 span 文本 */
lv/style_set_text-static(char* &), "This is span 4. ");
/* 设置 span 文本颜色 */
lv-style_set_text_color(&span->style, lv_palette_main(LV_PALETTE.Green));
/* 设置 span 文本字体 */
lv-style_set_text.Font(&span->style, &lv_font_montserrat_10);
/* 设置 span 文本格式 */
lv-style_set_text_decor(&span->style, LV_TEXT_DECOR_UNDERLINE);
/* 新建 span */
span = lv_spangroup_new-span(spangroup);
/* 设置 span 文本 */
...
微调器（spinbox）提供基于数值的输入方式，可通过按钮或编码器逐步调整数值，常用于设置需精确控制的参数。
...
void lv_spinbox_set_digit_format(lv_obj_t * spinbox, uint8_t digit_count, uint8_t separator_pos);
void lv_spinbox_set_value(lv_obj_t * spinbox, int32_t value);
void lv_spinbox_set_step(lv_obj_t * spinbox, uint32_t step);
void lv_spinbox_set_rollover(lv_obj_t * spinbox, bool en);
```

- `digit_count` 指总位数，`separator_pos` 指小数点前的位数（0 表示整数）。
- `lv_spinbox_set_rollover` 打开后，当增加/减少超出范围时会循环到另一端。

## 35.2 示例：按钮控制微调器

```c
static void spinbox_btn_event_cb(lv_event_t * e)
{
    lv_obj_t * btn      = lv_event_get_target(e);
    lv_obj_t * spinbox  = lv_event_get_user_data(e);

    if(btn == lv_obj_get_child(btn, 0)) return; /* 无需处理 */

    if(strcmp(lv_label_get_text(lv_obj_get_child(btn, 0)), "+") == 0) {
...
void lv_example_spinbox_basic(void)
{
    lv_obj_t * spinbox = lv_spinbox_create(lv_screen_active());
    lv_spinbox_set_range(spinbox, -1000, 1000);
    lv_spinbox_set_digit_format(spinbox, 5, 2);         /* 5 位，总共 3 位小数 */
    lv_spinbox_set_step(spinbox, 5);
    lv_obj_center(spinbox);

    /* “+” 按钮 */
    lv_obj_t * btn_plus = lv_btn_create(lv_screen_active());
    lv_obj_align_to(btn_plus, spinbox, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
    lv_obj_add_event_cb(btn_plus, spinbox_btn_event_cb,
                        LV_EVENT_CLICKED, spinbox);
    lv_obj_t * label_plus = lv_label_create(btn_plus);
    lv_label_set_text(label_plus, "+");

    /* “-” 按钮 */
    lv_obj_t * btn_minus = lv_btn_create(lv_screen_active());
    lv_obj_align_to(btn_minus, spinbox, LV_ALIGN_OUT_LEFT_MID, -10, 0);
    lv_obj_add_event_cb(btn_minus, spinbox_btn_event_cb,
                        LV_EVENT_CLICKED, spinbox);
...
1. 创建微调器并设置范围、数字格式、步进值；
2. 创建两个按钮，通过 `lv_spinbox_increment/decrement` 控制数值变化；
3. 可结合串口打印或标签显示当前数值。

## 35.3 其他接口

- `lv_spinbox_step_next` / `lv_spinbox_step_prev`：改变当前可调节的位数（光标位置）。
- `lv_spinbox_set_cursor_pos`：手动指定光标所在位。
- `lv_spinbox_get_value`：获取当前值（整数形式）。
- `lv_spinbox_set_rollover`：开启循环滚动。
...
用户需要创建加载器部件，可调用lv_spinner_create(parent, time, arc_length)函数，该函数有三个形参，parent形参表示部件的父类，time形参表示指示器旋转一圈的时间（以毫秒为单位），arc_length形参表示指示器的长度。
...
{ /*根据活动屏幕宽度选择字体*/ if (scr_act_width() <= 480) { font = &lv_font_montserrat_14; } else { font = &lv.Font_montserrat_20; } /*加载标题标签*/ label_load = lv_label_create(lv_screen_active()); lv_label_set_text(labelLabel_load, "LOADING..."); lv_obj_set_style_text_Font.label_load, font, LV_STATE_DEFAULT); lv_obj_align(label_load, LV_ALIGN_CENTER, 0, scr_act_height() / 10);   
}   
/*  
* @brief 加载器显示  
* @param 无  
...
{ spinner = lv_spinner_create(lv_screen_active(), 1000, 60); /*创建加载器*/ /*设置位置*/ lv_obj_align(spinner, LV_ALIGN_CENTER, 0, -scr_act_height() / 15);
```
...
```c
void lv_mainstart(void)   
{ /\*创建选项卡视图\*/ lv_obj_t\* tabview  $=$  lv_tabview_create(lv_screen_active(),LV_DIR_TOP,50); lv_obj_t\* tab1  $\equiv$  lv_tabview_add_tab(tableview，"Tab1");/\*添加选项卡1\*/ lv_obj_t\* tab2  $\equiv$  lv_tabview_add_tab(tableview，"Tab2");/\*添加选项卡2\*/
```
...
*/ static void lv_example_tabview(void)
{
    ...
    /* 第一部分：选项卡（基础） */
    /* 创建选项卡 */
    lv_obj_t *tabview = lv_tabview_createlv_screen_active(), LV_DIR_TOP, scr_act_height() / 6); /* 创建选项卡 */
...
```c
/\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\* /\*1、按钮\*/ /\*获取按钮部分\*/ lv_obj_t \*BTN = lv_tabview_get_tab_bsns(tableview); /\*未选中的按钮\*/ /\*设置按钮背景颜色：橙色\*/ lv_obj_set_style_bg_color(bg_t,lv_color_hex(0xb7472a), LV_PART_ITEMs|LV_STATE_DEFAULT); /\*设置按钮背景透明度\*/ lv_obj_set_style bg_opa(bg_t,200,LV_PART_ITEMs|LV_STATE_DEFAULT); /\*设置按钮文本颜色：白色\*/ lv_obj_set_style_text_color(bg,lv_color_hex(0xf3f3f3), LV_PART_ITEMs|LV_STATE_DEFAULT); /\*选中的按钮\*/ /\*设置按钮背景颜色：灰色\*/ lv_obj_set_style_bg_color(bg,lv_color_hex(0xe1e1e1), LV_PART_ITEMs|LV_STATE_CHECKED); /\*设置按钮背景透明度\*/ lv_obj_set_style bg_opa(bg,200,LV_PART_ITEMs|LV_STATE_CHECKED); /\*设置按钮文本颜色：橙色\*/ lv_obj_set_style_text_color(bg,lv_color_hex(0xb7472a), LV_PART_ITEMs|LV_STATE_CHECKED); /\*设置按钮边框宽度为0\*/ lv_obj_set_styleborder_width(bg,0,LV_PART_ITEMs|LV_STATE_CHECKED); /\*2、主体\*/ /\*获取主体部分\*/ lv_obj_t \*obj  $=$  lv_tabview_get_content(tableview); /\*设置背景颜色：白色\*/ lv_obj_set_style_bg_color(obj,lv_color_hex(0xFFFFFF),LV_STATE_DEFAULT); /\*设置背景透明度\*/ lv_obj_set_style bg_opa(obj,255,LV_STATE_DEFAULT);
```
...
用户需要创建平铺视图部件，可调用lv_tileview_create函数，值得注意的是，在默认情况下，该部件被创建出来后并没有任何页面。需要往平铺视图部件中添加页面，可以调用lv_tileview_add_tile函数进行设置，该函数的第二、三个形参代表页面所在的行列坐标（见图38.1.1），第四个形参代表页面切换的方向，该滑动方向的可选参数如下所示：
...
```c
lv_label_set_text(label_1, "Page_1"); /*设置文本内容*/lv_obj_set_style_text_Font.label_1,font，LV_STATE_DEFAULT); /*设置字体*/lv_obj_center labelled_1); /*设置位置*/lv_obj_t *label_2 = lv_label_create(tile_2); /*创建标签*/lv_label_set_text labelled_2, "Page_2"); /*设置文本内容*/lv_obj_set_style_text_Font labelled_2,font，LV_STATE_DEFAULT); /*设置字体*/lv_obj_center labelled_2); /*设置位置*/lv_obj_t *label_3 = lv_label_create(tile_3); /*创建标签*/lv_label_set_text labelled_3, "Page_3"); /*设置文本内容*/lv_obj_set_style_text_Font labelled_3,font，LV_STATE_DEFAULT); /*设置字体*/lv_obj_center labelled_3); /*设置位置*/lv_obj_remove_style tileview，NULL，LV_PART_SCROLLBAR); /*移除滚动条*//\*左侧状态栏\*/lv_obj_t *label_left  $=$  lv_label_create lv_screen_active(); /*创建标签\*/lv_label_set_text(label_left, "AM 8:30"); /*设置文本内容\*/lv_obj_set_style_text_Font.label_left,font,LV_STATE_DEFAULT); /*设置字体\*/lv_obj_align labelled_left，LV_ALIGN_TOP_LEFT，10，10）; /*设置位置\*//\*右侧状态栏\*/lv_obj_t *label_right  $=$  lv_label_create (lv_screen_active()); /*创建标签\*/lv_label_set_text(label_right，LV_SYMBOL_WIFI"  $80 \%$  LV_SYMBOL_BATTERY_3); /*设置文本内容\*/\*设置字体\*/lv_obj_set_style_text_Font.label_right，font，LV_STATE_DEFAULT); /lv_obj_align labelled_right，LV_ALIGN_TOP_RIGHT，-10，10）; /*设置位置\*/}
```
...
```c
lv_obj_t *lv_win_create lv_obj_t *parent, lv_coordinate_t header_height);
```
...
/* 设置大小 */
lv_obj_set_size(slider.audio, scr_act_width()/3, scr_act_height()/30);
/* 设置位置 */
lv_obj_align(slider.audio, LV_ALIGN_CENTER, 15, -scr_act_height()/14);
/* 设置当前值 */
...
```javascript
/\*设置指示器颜色\*/  
lv_obj_set_style_bg_color(slider.audio，lv_color.hex(0xc3c3c3)，LV_PART_INDICATOR);  
/\*移除旋钮\*/  
...
/\*设置字体\*/  
lv_obj_set_style_text_Font[label.audio，font，LV_STATE_DEFAULT);  
/\*设置位置\*/  
lv_obj_align_to(label.audio，slider.audio，LV_ALIGN_OUT_LEFT_MID，-scr_act_width(/40,0);  
/\*闹钟音量滑块\*/  
...
/\*设置主体颜色\*/  
lv_obj_set_style_bg_color(slider_bell，lv_color_hex(0x787c78)，LV_PART_MAIN);  
/\*设置指示器颜色\*/  
lv_obj_set_style_bg_color(slider_bell，lv_color_hex(0xc3c3c3)，LV_PART_INDICATOR);  
/\*移除旋钮\*/  
...
/\*设置位置\*/  
lv_obj_align_to labelled_bell，slider_bell，LV_ALIGN_OUT_LEFT_MID，-scr_act_width(/40,0);
```
...
```c
void lv_animg_set_src(lv_obj_t * obj, lv_image_dsc_t * dsc[], uint8_t num);
```
...
① `LV_MENU_HEADER_TOP_FIXED`：标题固定在顶部。  
② `LV_MENU_HEADER_TOP_UNFIXED`：标题位于顶部，可随内容滚动。  
③ `LV_MENU_HEADER_BOTTOM_FIXED`：标题固定在底部。
...
void lv_main_start(void)
{
    ...
    /* 第二步：创建菜单页面 */
    lv_obj_t * page = lv_menu_page_create(menu, "ATK");

    /* 第三步：设置菜单页面样式 */
    lv_obj_set_style_pad_hor(page,
                             lv_obj_get_style_pad_left(lv_menu_get_main_header(menu), 0),
                             0);
...
创建菜单页面之后，用户可以使用 `lv_menu_set_sidebar_page` 函数将其设置到侧边栏。该函数具有两个形参，第一个形参指向菜单的对象，而第二个形参指向菜单页面，如果该形参为 NULL，则该函数是用来清除侧边栏。本小节的知识和上小节知识类似，主要讲解菜单页面添加到侧边栏容器中。

接下来，我们以一个简单实例来讲解这小节的知识，如下源码所示：

```c
void lv_main_start_sidebar(void)
{
    /* 第一步：创建菜单部件 */
    lv_obj_t * menu = lv_menu_create(lv_screen_active());
    lv_menu_set_mode_root_back_btn(menu, LV_MENU_ROOT_BACK_BTN_ENABLED);
...
从上图可知：左边的侧边栏也是菜单页面，这个设置方法是调用 lv_menu_set_sidebar_page 函数设置，该菜单页面可以添加各种小部件来作为连接源(点击对象)，上图右边的是连接的页面，当用户点击侧边栏中的“Mechanics”标签可切换右边的任意菜单页面，这个菜单页面需要用户调用lv_menu_set_load_page_event进行连接。
...
<table><tr><td>函数</td><td>描述</td></tr><tr><td colspan="2">菜单部件设置函数</td></tr><tr><td>lv_menu_create()</td><td>创建菜单部件</td></tr><tr><td>lv_menu_page_create()</td><td>创建菜单页面</td></tr><tr><td>lv_menu_cont_create()</td><td>创建菜单容器</td></tr><tr><td>lv_menu_section_create()</td><td>创建空区域</td></tr><tr><td>lv_menu_separator_create()</td><td>创建分隔符</td></tr><tr><td>lv_menu_set_page()</td><td>设置菜单页面为主容器</td></tr><tr><td>lv_menu_set_sidebar_page()</td><td>设置菜单页面为侧边栏容器</td></tr><tr><td>lv_menu_set_mode_header()</td><td>设置标题模式</td></tr><tr><td>lv_menu_set_mode_root_back_btn()</td><td>设置根返回按键</td></tr><tr><td>lv_menu_set_load_page_event()</td><td>菜单页面连接</td></tr><tr><td colspan="2">菜单部件获取函数</td></tr><tr><td>lv_menu_get_cur_main_page()</td><td>当前主容器页面</td></tr><tr><td>lv_menu_get_cur_sidebar_page()</td><td>当前侧边栏页面</td></tr><tr><td>lv_menu_get_main_header()</td><td>获取主标题</td></tr><tr><td>lv_menu_get_main_header_back_btn()</td><td>获取主标题返回按键</td></tr><tr><td>lv_menu_get_sidebar_header()</td><td>获取侧边栏标题</td></tr><tr><td>lv_menu_get_sidebar_header_back_btn()</td><td>获取侧边栏返回按键</td></tr><tr><td>lv_menu_back_btn_is_root()</td><td>判断当前返回按键是否指向根页面</td></tr><tr><td>lv_menu_clear_history()</td><td>清除菜单历史</td></tr></table>
...
```c
static lv_result_t decoder_read_line( lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_coord_t len, uint8_t * buf)   
...
```c
for(int i = 0; i < len; i++)  
{lv_fs_read(&b->f, color_buf, 3, NULL);color_p[i] = lv_color_make(color_buf[2],color_buf[1], color_buf[0]);}break;  
case 16:for(int i = 0; i < len; i++)lv_fs_read(&b->f, color_buf, 2, NULL);uint16_t color16 = \* (uint16_t \*) color_buf;#define RGB565_R5(rgb565) ((rgb565 >> 11) & 0x1f)#define RGB565_G6(rgb565) ((rgb565 >> 5) & 0x3f)#define RGB565_B5(rgb565) ((rgb565 >> 0) & 0x1f)#define R5_2_R8(r5) (r5 << 3)#define G6_2_G8(g6) (g6 << 2)#define B5_2_B8(b5) (b5 << 3)color_p[i] = lv_color_make(R5_2_R8(RGB565_R5(color16)),G6_2_G8(RGB565_G6(color16)),B5_2_B8(RGB565_B5(color16));}break;  
case 8:for(int i = 0; i < len; i++){lv_fs_read(&b->f, color_buf, 1, NULL);uint8_t index = color_buf[0];rgbquad_t color = b->palette[index];color_p[i] = lv_color_make(color.red,color.green,color.blue);}break;  
case 4:for(int i = 0; i < len;){lv_fs_read(&b->f, color_buf, 1, NULL);for(int j = 0; (j < 8) && (i < len); j += 4, i++) {uint8_t index = (color_buf[0] >> (4 - j)) & 0x0f;
```

```txt
rgbquad_t color  $=$  b->palette[index]; color_p[i]  $=$  lv_color_make(color.red, color.green, color.blue); } 1 break; case 1: for(int i  $= 0$  ;i<len;} { lv_fs_read(&b->f,color_buf,1,NULL); for(int j  $= 0$  ;  $(\mathrm{j} <   8)$  && (i<len);j++,i++){ uint8_t index  $=$  (color_buf[0] >> (7 - j)) & 0x01; rgbquad_t color  $=$  b->palette[index]; color_p[i]  $=$  lv_color_make( color.red, color.green, color.blue); } 1 break; default: break; } return LV_RESULT_OK;
```
...
\*/   
void lv_mainstart()   
{ lv_obj_t \*label  $=$  lv_label_create(lv_screen_active()); lv_label_set_text(label,"BMP_Decoder"); lv_obj_set_style_text_color.label,lv_palette_main(LV_PALETTE_RED), LV_STATE_DEFAULT); lv_obj_set_style_text_Font.label,&lv_font_montserrat_32,LV_STATE_DEFAULT); lv_obj_align_to labelledNull,LV_ALIGN_TOP_MID,0,0); lv_obj_set_style_bg_colorlv_screen_active(),lv_palette_main(LV_PALETTE_BLUE), LV_STATE_DEFAULT); img  $=$  lv_image_create(lv_screen_active()); /\*创建图像\*/ lv_image_set_src(img，drawimg[O].img_path); /\*设置图像源\*/ lv_obj_align_to(img，NULL，LV_ALIGN_CENTER，0，0）;/\*设置对齐模式\*/   
}
```
...
复制“LVGL例程5LVGL文件系统使用”工程并重命名为“LVGL例程42PNG图片读取”。打开工程并在工程中创建Middlewares/lvgl/src/png工程管理项分组，在该组添加lodeppng.c和lv_png.c文件，这些文件在LVGLGUI/lvgl/src/extra/libs/png路径下定义的。如下图所示：
...
{ lv_obj_t \* img  $=$  lv_image_create(parent); lv_image_set_src(img，path); lv_obj_align_to(img，NULL，LV_ALIGN_CENTER,0,0); return img;   
}   
/\*\*   
\* @brief LVGL程序入口   
\* @param 无   
...
\*/   
void lv_mainstart()   
{ lv_png_init(); /\*初始化PNG解码库\*/ lv_obj_t \*label  $=$  lv_label_create(lv_screen_active()); lv_label_set_text(label,"PNG_Decoder"); lv_obj_align_to labelledNull,LV_ALIGN_TOP_MID,0,0); /\*创建PNG文件\*/ img  $=$  lv_png_create_from_file(lv_screen_active(),PNG_PATH[0].img_path);   
1
```
...
9 lv_obj_t * wp;  
10  
11 wp = lv_image_create(lv_screen_active());  
12 /* Assuming a File system is attached to letter 'A'  
13 * E.g. set LVUSE_FS_STDIO 'A' in lv_conf.h */  
14 lv_image_set_src(wp, "A:lvgl/examples/libssjpg/small_image.sjpg");  
15 }  
16  
17 #endif
```
...
void lv_mainstart()   
{ /\*创建标签\*/ lv_obj_t \*label  $=$  lv_label_create(lv_screen_active()); /\*设置标签颜色\*/ lv_label_set_text(label, "JPEG_Decoder"); /\*设置文本颜色\*/ lv_obj_set_style_text_color.label,lv_palette_main(LV_PALETTE_RED), LV_STATE_DEFAULT); /\*设置文本字体\*/ lv_obj_set_style_text_Font.label,&lv_font_montserrat_32,LV_STATE_DEFAULT); /\*设置顶部中间对齐\*/ lv_obj_align labelled,LV_ALIGN_TOP_MID,0,0); /\*设置背景颜色\*/ lv_obj_set_style_bg_colorlv_screen_active(),lv_palette_main(LV_PALETTE_BLUE), LV_STATE_DEFAULT); /\*创建image部件\*/ lv_obj_t \*img  $=$  lv_image_create(lv_screen_active());
```

```txt
/\*设置图像源\*/  
lv_image_set_src(img，"0:/PICTURE/JPEG/SIM900A.jpg");  
/\*中间对齐\*/  
lv_obj_align(img,LV_ALIGN Center,0,0);
```
...
extern int has_next;   
void lv_mainstart(void)   
{ lv_obj_t *label; label  $=$  lv_label_create(lv_screen_active()); lv_label_set_text(label, "GIF_Decoder"); lv_obj_set_style_text_color.label, lvpalette_main(LV_PALETTE_RED), LV_STATE_DEFAULT); lv_obj_set_style_text_Font.label,&lv_font_montserrat_32,LV_STATE_DEFAULT); lv_obj_align_to labelled, NULL,LV_ALIGN_TOP_MID,0,0); lv_obj_set_style_bg_color lv_screen_active(),lvpalette_main(LV_PALETTE_BLUE), LV_STATE_DEFAULT); lv_obj_set_style_bg_color lv_screen_active(),lvpalette_main(LV_PALETTE_BLUE), LV_STATE_DEFAULT); img  $=$  lv.gif_create(lv_screen_active()); lv.gif_set_src(img,GIF_PATH-image).img_path); lv_obj_align_to(img,NULL,LV_ALIGN_CENTER,0,0); lv_timer_createLv_my_timer,10,image);   
}
```
...
{  
    ...
    {  
        image = 0; /* 重新开始展示 */  
        img = lv.gif_create lv_screen_active();  
...
```c
} timer->user_data  $\equiv$  img; /* 设置任务数据等于获取的图片数据*/ lv_obj_align_to(img,NULL,LV_ALIGN_CENTER,0,0); 1
```
...
```c
lv_result_t lv_qrtc_update( lv_obj_t * qrtc, const void * data, uint32_t data_len)
```
...
返回值：LV_RESULT_OK:没有错误；LV_RESULT_INVALID:错误。
...
void lv_mainstart(void)   
{ /\*创建一个标签\*/ lv_obj_t \*label_time  $=$  lv_label_create(lv_screen_active()); /\*设置标签的文本\*/ lv_label_set_text(label_time,"ALIENTEK_QR"); /\*设置标签的文本字体颜色\*/ lv_obj_set_style_text_color.label_time,lv_palette_main(LV_PALETTE_RED), LV_STATE_DEFAULT); /\*设置标签的文本字体\*/ lv_obj_set_style_text_Font.label_time,&lv_font_montserrat_32, LV_STATE_DEFAULT); /\*设置标签的顶部中间对齐\*/ lv_obj_align labelled_time,LV_ALIGN_TOP_MID,0,0);
```

```c
/\*创建一个lcddev.width/2的二维码\*/  
lv_obj_t \*qr  $=$  lv_qrcode_create(lv_screen_active(),lv_obj_get_width(lv_screen_active()）/2,lv_color(hex3(0x33f),lv_color.hex3(0xeef));  
/\*设置数据\*/  
...
在LVGL中，使用Flex布局有两种方式，第一种是使用lv_obj_set_layout函数方法设置对象启用Flex布局，第二种是使用lv_obj_set_flex_flow函数启用Flex布局。下面笔者以一个简单实例来讲解本小节的内容，如下所示：
