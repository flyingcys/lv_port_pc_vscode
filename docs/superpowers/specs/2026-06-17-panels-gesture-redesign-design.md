...
| 面板高度 | ~~内容自适应高度~~ → **改为全屏铺满**（screen_h，2026-06-17 用户看图后改定：上下滑出的面板占满整屏，而非只有内容那么高） |
...
### 4.1 全屏铺满（2026-06-17 修订）

> 原定「内容自适应高度」，用户看到实现后改为**全屏铺满**：上下滑出的面板展开后占满整个屏幕。

- 面板 `height = screen_h`（全屏），`radius = 0` 去圆角防屏角露壁纸；半透明毛玻璃近似下隐约透出壁纸/状态栏。
- `H = screen_h`，全开/全关位置与 §3 钳制公式均基于 `screen_h`（control 全关 y=-screen_h、全开 y=0；notify 全关 y=screen_h、全开 y=0）。
- 控制中心用弹性占位（flex_grow）把把手顶到屏幕底部；通知中心把手在顶部。
- **层级**：全屏展开时面板会盖住边缘感应带，故新增 `ir2_panels_bring_to_front()` 在感应带创建后把面板置顶 —— 展开时把手可点；关闭时面板在屏外不挡感应带的开合手势。
